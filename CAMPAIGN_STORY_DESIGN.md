...
**Not decided yet:** exact numeric budgets per region, and whether the capstone gets a bonus choice at all or skips it since there's no region after it to carry a bonus into.
...
- All six companions' full personal arcs and dialogue (Ch1-3 beats, chapter-boundary reactions, homecoming beats) are still just one-line hooks — names and mirror-relationships are locked, the writing itself isn't done. Kevin isn't a writer by his own description, so the plan is a full first-draft pass on all of it, treated as punch-up-able rough material rather than final copy.
