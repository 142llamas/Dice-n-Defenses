...
| D-232: 4 new original spellcasting focus items — Apprentice's Wand, Gnarled Staff, Wizard's Spellbook, Scrying Crystal — filling out Kevin's item-8 ask ("spellbook, staff, wand, crystal ball") alongside the 4 pre-existing foci (Holy Symbol/Arcane Focus/Druidic Totem/Component Pouch, D-193, never previously given their own row here — covered by the general equipment-catalogue disclosure two rows up) | N/A — original content | N/A | No — not SRD-derived | Generic fantasy flavor names/descriptions and flat +1 AC/attack-damage numbers, no SRD item text or named-magic-item concept reused; checked directly against `magicItems.ts`'s existing "Wand of Magic Missile"/"Staff of Healing" (an unrelated `chargedSpell` mechanic) before naming these, to avoid a collision | `src/game/data/equipment.ts` |
| D-253: Dorian Wick's new side mission, "What the Causeway Kept" — its name, description, 3-wave content (`side-dorian-wave-1..3`), and the choice of `marauder` (an already-original enemy) as its finale — built when his recruitment moved from Shattered Causeway's Chapter 1 (demoted to optional content) to his own standalone side mission, matching the other 6 Pool A companions' missions (Phase 27/D-179–D-180 rows above logged region-chapter content but never separately logged the individual side-mission rows themselves — a pre-existing gap this row also covers going forward, not something to perpetuate) | N/A — original content | N/A | No — not SRD-derived | Every wave composition, enemy count/timing, gold number, and description line is invented for this project; `marauder` and `CAUSEWAY_MAP` were already original content logged in earlier rows — no new enemy, equipment, or map was added | `src/game/data/campaigns.ts` |
| D-235: real SRD 5.2.1 weapon-proficiency-per-class table (which of the 12 classes get Simple weapons only vs. full Martial access, plus Monk's Light-property and Rogue's Finesse-or-Light named exceptions) — verified during implementation directly from a 2024-SRD mirror, not assumed from memory (same "verify real docs, don't assume" discipline this project already applied once for Tough/Lucky/Athlete and the 4-background count, see the two rows above) | SRD 5.2.1 | CC BY 4.0 | Yes, before public release | No game text copied — only the proficiency STRUCTURE (which weapon categories/properties each class can use) is SRD-derived; the filtering/hiding UI behavior built on top of it is original engineering. The 2024 SRD's Cleric (Divine Order)/Druid (Primal Order) optional level-1 choice to gain martial proficiency isn't modeled — `classes.ts` never built that choice mechanic (only Divine Domain, a different level-1 choice), so both classes are recorded at their base Simple-only line, a deliberate scope call | `src/game/data/proficiencies.ts` |
