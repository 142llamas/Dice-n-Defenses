...
export const ENEMY_DEFINITIONS: Record<string, EnemyDefinition> = {
  ...
  // D-177 (KI-098 item 13): Saltmere Shallows' Ch1 miniboss stand-in.
  // CAMPAIGN_STORY_DESIGN.md §4's "returning miniboss" mechanic (whichever
  // miniboss the player spared in an earlier region) depends on world-flag
  // state from other regions that aren't playable content yet — this is the
  // design doc's own documented fallback for exactly that case: "a nameless
  // tide-wretch... mechanically a miniboss-tier reskin... but with no
  // loreText, no name callout, no story weight. It's explicitly a wall, not
  // a character." Deliberately no `loreText` — that omission is the point.
  "tide-wretch": {
    id: "tide-wretch",
    name: "Tide-Wretch",
    maxHealth: 32,
    ...
    rewardGold: 21,
    abilities: [],
    assetKey: "enemy-tide-wretch",
    role: "miniboss",
  },

  // D-185 (KI-098 item 13): Sorrel Thane's own "Lost" outcome, per
  // CAMPAIGN_STORY_DESIGN.md §6's branch chain — takes Saltmere Ch1's
  // returning-miniboss slot outright (see ReturningMinibossSystem), ahead
  // of a spared miniboss. Stat block cloned verbatim from tide-wretch —
  // zero new balance work — but unlike the wall-only tide-wretch fallback,
  // this one keeps a real name and loreText on purpose: she was a
  // character, not a placeholder.
  "corrupted-sorrel": {
    id: "corrupted-sorrel",
    name: "Sorrel Thane, Lost",
    maxHealth: 32,
    armorClass: 13,
    attackBonus: 5,
    savingThrowBonus: 5,
    ...
    rewardGold: 21,
    abilities: [],
    assetKey: "enemy-corrupted-sorrel",
    role: "miniboss",
    loreText:
      "She stopped answering to Sorrel somewhere in the Vale. What's left still moves like a warden — still stands between something and the people behind it — it just can't remember anymore which side it's guarding.",
  },
  ...
  bannerbearer: {
    ...
    movementType: "ground",
    rewardGold: 6,
    auraBuff: { radiusTiles: 2, movementBonus: 2 }, // D-172: rescaled from 5ft/tile (was 1)
    abilities: [],
  ...
  // ===== D-188: The Nameless Throne (CAMPAIGN_STORY_DESIGN.md §5) =====
  // Six reskins garrisoning the capstone, three per ending. Each is a
  // verbatim stat clone of an existing enemy (only id/name/assetKey/
  // loreText/color differ) — the same "corrupted-sorrel from tide-wretch"
  // precedent D-185 established. No new mechanics, no new balance work.

  // Ashen Sovereign garrison — fire-touched reskins of Warden/Hexer/Gravemaw.
  "ember-thane": {
    id: "ember-thane",
    name: "Ember Thane",
    maxHealth: 10,
    ...
    rewardGold: 5,
    abilities: [],
    assetKey: "enemy-ember-thane",
    role: "minion",
    loreText: "Whatever office the Ember Thane once held, only the ash-cloak and the certainty of rank survived it.",
  },
  "cinder-adept": {
    id: "cinder-adept",
    name: "Cinder Adept",
    ...
    rewardGold: 4,
    abilities: [],
    assetKey: "enemy-cinder-adept",
    role: "minion",
    loreText: "It still casts the way it was taught. It has forgotten every reason it was taught to.",
  },
  "ashbound-honor-guard": {
    id: "ashbound-honor-guard",
    name: "Ashbound Honor Guard",
    maxHealth: 34,
    armorClass: 14,
    attackBonus: 5,
    savingThrowBonus: 5,
    ...
    rewardGold: 22,
    abilities: [],
    assetKey: "enemy-ashbound-honor-guard",
    role: "miniboss",
    loreText: "It still stands the way an honor guard stands. It has no memory of what it was ever guarding.",
  },

  // Hollow Empress garrison — drowned/withered reskins of Warden/Blightcaller/The Husk.
  "drowned-thane": {
    id: "drowned-thane",
    name: "Drowned Thane",
    maxHealth: 10,
    ...
    rewardGold: 5,
    abilities: [],
    assetKey: "enemy-drowned-thane",
    role: "minion",
    loreText: "Whatever office the Drowned Thane once held, only the waterlogged cloak and the certainty of rank survived it.",
  },
  "hollow-caller": {
    id: "hollow-caller",
    name: "Hollow Caller",
    ...
    savingThrowAttackDC: 12,
    abilities: [],
    assetKey: "enemy-hollow-caller",
    role: "minion",
    loreText: "It still calls out the way it was taught. It has forgotten every name it was ever calling for.",
    damageResistances: ["poison"],
  },
  "drowned-honor-captain": {
    id: "drowned-honor-captain",
    name: "Drowned Honor Captain",
    maxHealth: 50,
    armorClass: 10,
    attackBonus: 1,
    savingThrowBonus: 2,
    ...
    abilities: [],
    assetKey: "enemy-drowned-honor-captain",
    role: "miniboss",
    loreText: "It still stands the way a captain stands. It has no memory of what it was ever guarding.",
    damageImmunities: ["poison"],
  },
...
export const ENEMY_COLORS: Record<string, number> = {
  ...
  // D-177: tidal-toned, echoing Tidelord's own color — a nameless wretch of the same waters.
  "tide-wretch": 0x3a6a7a,
  // D-185: the same tidal palette as tide-wretch (same returning-encounter
  // slot), shifted toward a sickly violet-grey — corrupted, not just wet.
  "corrupted-sorrel": 0x6a5a7a,
  ...
  // D-188: The Nameless Throne garrison reskins — warm ember/orange family
  // for the Ashen Sovereign's escort, cold violet-grey family (echoing
  // corrupted-sorrel's own palette note) for the Hollow Empress's escort.
  "ember-thane": 0xd06a20,
  "cinder-adept": 0xe08a3a,
  "ashbound-honor-guard": 0xb0501a,
  "drowned-thane": 0x4a5a7a,
  "hollow-caller": 0x6a7a9a,
  "drowned-honor-captain": 0x3a4a6a,
