...
## [Unreleased] — 0.2.0-dev — Full-codebase cleanup pass (D-255) + a real mercy-tally bug fix (D-254)

A "very thorough" file-by-file, line-by-line cleanup pass across the whole
project (149 `src/` files, 113 test files, and every doc), run via 14
parallel audit agents. See `DECISIONS.md` D-254/D-255 for the complete
accounting — summary here.

Fixed:
- **A real gameplay bug (D-254)**: `NamelessThroneSystem.computeMercyTally`
  was silently counting a skipped, now-optional Shattered Causeway as
  "showed no mercy" toward the capstone's ending. Fixed to exclude an
  unplayed Causeway from the tally entirely (Kevin's own choice among 3
  presented options). See `KNOWN_ISSUES.md` KI-202.
- 9 stale comments/doc-comments across `InitiativeSystem.ts`,
  `WorldFlagSystem.ts`, `ProgressionSystem.ts`, `RestSystem.ts`,
  `BattleScene.ts`, `Hero.ts`, `data/campaigns.ts` (×2), `data/difficulty.ts`,
  `UnlockMissionSystem.ts`, and `CharacterCreationScene.ts` — all referenced
  removed/superseded systems or a nonexistent method/scene.

Removed (confirmed dead — zero production callers, verified by grepping the
whole repo, not just the assigned file):
- `GameMap.describe()`, `SpellPreparationSystem.isValidSelection()`,
  `ProgressionSystem.levelsSoFar`, `BuildSystem.removeAt()`, and two dead
  `export type { GridPosition }` re-exports (`data/structures.ts`,
  `data/enemies.ts`).

Changed (duplicated logic consolidated onto one shared implementation):
- `MapBuilderScene`'s tile-size formula now calls the existing
  `GridSystem.computeFittedTileSize` instead of hand-duplicating it.
- `CombatSystem.chooseTarget`/`attackArea` now share one
  `compareTargetPriority` comparator instead of two copies of the same
  nearest/health/id tie-break.
- `SpellcastingSystem`'s sparse-table lookup helper moved to
  `CharacterSystem.ts` (exported) and is now shared by both files' own
  per-level lookups instead of 3 separate copies.
- `ClassProgressionSystem`/`ProficiencySystem` now call
  `SpellcastingSystem.isSpellcaster` instead of reimplementing it;
  `ClassProgressionSystem`/`LevelUpPlanSystem` now call
  `CharacterSystem.featuresAtLevel` instead of reimplementing it;
  `gearPickerView.ts`/`BattleScene.ts` now call `LootSystem.isPotionId`
  instead of reimplementing it; `BattleScene.ts` now calls the existing
  `BuildSystem.trapIsSingleUseAt` instead of computing it independently;
  `CharacterCreationScene`'s exact-duplicate private `maxCastableSpellLevel`
  was deleted in favor of the shared `SpellPreparationSystem` export.
- 2 exact-duplicate test assertions removed from `campaign.test.ts`
  (already covered, more precisely, by `enemyRoster.test.ts`).

Documentation: `CLAUDE.md` and `README.md` were both badly stale (README
still described the classic 4-hero/10-wave MVP with 157 tests) — both
rewritten to reflect the current game. Also corrected:
`PHASE_12_MULTIPLAYER_FEASIBILITY.md`/`CAMPAIGN_STORY_DESIGN.md` (stale
"design only" banners), `SOURCE_OF_TRUTH.md` (a repo-note wrongly claiming
2 already-resolved items were still open), `FIREBASE_SETUP.md` (framed as
not-yet-done), `ASSET_PLAN.md` (missing system), `PARTY_CREATION_OVERHAUL_PLAN.md`
(missing status banner), `package.json`'s description, and — with Kevin's
explicit sign-off per this project's own doc-history conventions —
`DECISIONS.md` (one duplicated paragraph removed, 3 missing cross-reference
notes added), `KNOWN_ISSUES.md` (3 stale/contradictory checklist items
corrected), and `PROJECT_STATUS.md` (a false header claim fixed, a
fossilized ~467-line MVP-era snapshot collapsed to a pointer).
...
`uiTheme.ts`/`uiScrollList.ts` components every sibling scene uses) needs a
browser to verify, so it's a recommendation for a future session.

## [Unreleased] — 0.2.0-dev — Batch H: chapter-select submenu + the 19-chapter campaign restructure (D-253) — CLOSES THE ENTIRE 2026-09-09 19-ITEM PLAYTEST LIST

Items 12/13, the last item on Kevin's 2026-09-09 19-item playtest list — the
single most disruptive item on it (content cuts, chapter renumbering, a new
leveling model). Emberford Reach loses its old Chapter 3 (folds 4→3
chapters); Shattered Causeway is demoted to optional/non-mandatory content
(5 regions × mostly-4 chapters = 19 required chapters total).

Added:
- `src/game/scenes/ChapterSelectScene.ts` — a new chapter-picker submenu
  between a region card click and the existing mission-routing flow, listing
  every chapter of a region with locked/unlocked/completed status and replay
  of anything already cleared.
- `src/game/scenes/missionRouting.ts` — the shared 2-branch routing logic
  (unlock-mission vs. straight-to-bonus-choice) `CampaignSelectScene` and the
  new `ChapterSelectScene` both use.
- Dorian Wick's new side mission ("What the Causeway Kept") — he now
  recruits via his own mission (Pool A) instead of Shattered Causeway's
  Chapter 1 (Pool B), matching the other 6 side-mission companions.

Changed:
- Campaign leveling: a chapter clear now grants exactly ONE character level
  (capped at 20, order-independent — regions already unlock in parallel with
  no forced sequence), replacing the old continuous per-wave ramp toward a
  fixed per-chapter level band. Free Play's own leveling is completely
  unaffected. Gated against farming free levels by replaying an
  already-cleared chapter.
- Campaign boss/enemy scaling now keys off the party's actual accumulated
  campaign level instead of each chapter's static level band — a companion
  fix the leveling change required, since the old band-based scaling was
  only ever "close enough" thanks to the per-wave ramp this change removes.
- `REGION_CAMPAIGN_IDS` (the Nameless Throne capstone gate + legacy-save
  level backfill) drops Shattered Causeway — it stays a full, playable
  campaign, just no longer mandatory.

Fixed (found during this batch, not part of the original ask):
- `BattleScene.showMirrorBossReactionIfAny` hardcoded chapter index 3 as
  every region's finale — silently broken for Emberford Reach's new 3-chapter
  shape; now derives the finale index correctly for every region.

Known, accepted narrative loss (not a bug): Shattered Causeway's finale no
longer plays Dorian Wick's "homecoming beat" dialogue — moving him to a side
mission (required so existing tests treat him like every other Pool A
companion) means he doesn't get that beat. See `KNOWN_ISSUES.md` KI-201 for
the full playtest checklist.

## [Unreleased] — 0.2.0-dev — Batch F: real mid-battle autosave for Campaign and Free Play (D-252)

Item 18 of Kevin's 2026-09-09 19-item playtest list — the last item on the
whole list. Two scope forks confirmed with Kevin before design: checkpoint
after each wave clear, covering both Campaign and Free Play.

Added:
- `src/game/systems/AutosaveSystem.ts` — a rotating pool of up to 3
  mid-battle checkpoints, upserted by a stable per-run id (so repeated
  checkpoints of the same run never duplicate, and only a genuinely new
  run evicts the oldest other one once full).
- `BattleScene` checkpoints via the previously-unused `BattleStateSnapshot`
  (`captureBattleState`/`restoreBattleState`, built in Phase 12.1/D-101 for
  multiplayer-sync feasibility) right after each wave clears, for real
  Campaign/Free-Play runs only (not Co-op/Test Mode).
- `ModeEntryScene` gains a "Continue" button per mode (shown only when an
  in-progress run exists); `LoadGameScene` gains a third `"autosave"` list
  mode with "Resume"/"Delete" actions, resuming straight into `BattleScene`
  without re-entering Character Creation.

Changed:
- `BattleScene.create()` gains an additive resume branch — the fresh-start
  path is unchanged; a resume restores heroes/wave/gold/structures/rests
  from the checkpoint and skips the one-time pre-wave-1 setup (chapter
  intro, region bonus, fast-forward choices) that's always already resolved
  by the time a checkpoint can exist.
- "Disable actions while saving" / "warn if it takes >0.5s" (both literal
  asks from item 18): a new `autosaving` flag joins `inputLocked()`'s
  existing OR-chain; a slow write logs a warning via the combat log — real,
  correct plumbing that won't visibly fire until a slower persistence layer
  (e.g. a future cloud sync) exists, same as today's synchronous
  `localStorage` write.

Known, accepted gaps (not part of this decision, see `DECISIONS.md` D-252):
an active summon won't survive a resume (`SummonSystem` was already a
documented `BattleStateSnapshot` gap); an already-claimed treasure tile
could pay out a second time after a resume (`consumedTreasureTiles` was
already documented as not persisted). See `KNOWN_ISSUES.md` KI-200 for the
full playtest checklist — this is the riskiest change of the whole 19-item
arc, touching `BattleScene.create()`'s core setup.

## [Unreleased] — 0.2.0-dev — Batch G: "Reset to Default" for Settings and Controls (D-251)

Item 19 of Kevin's 2026-09-09 19-item playtest list. Defaults already
existed as named constants; nothing wrote them back until now.

Added:
- `SettingsSystem.resetSettings` / `KeyBindingSystem.resetKeyBindings` —
  pure functions writing each system's `DEFAULT_*` constant back to
  storage and returning it, mirroring the existing `load`/`save` shape.
- `SettingsScene` gains a "Reset to Default" button (Controls section,
  above Back), guarded by a confirmation prompt copying `PauseMenuScene`'s
  own message/Confirm/Cancel pattern.

Changed:
- Confirming Reset re-applies volume/mute live to `AudioManager`, and — in
  the in-battle pause-menu overlay entry mode — also resets the live
  `BattleScene`'s own Game Speed, not just the stored value.

Deliberately not built: item 19a's named/savable setting profiles (Kevin's
own softer "I wouldn't hate..." phrasing) — no profile concept exists
anywhere today; flagged as an open nice-to-have, not part of this pass.

See `DECISIONS.md` D-251 for the full design and `KNOWN_ISSUES.md` KI-199
for the playtest checklist.

## [Unreleased] — 0.2.0-dev — Reconciled Batch E's real gaps: equipment-bonus recipient choice, value-comparable pricing, player-placed free structures (D-250)

A new `PLAYTEST_2026-09-09_BATCHES.md` tracker (this session) surfaced that
D-248's own "closes Batch E" claim was wrong — items 6b/6c/15 were never
built. This closes all three.

Added:
- `RegionBonusChoiceScene` now asks "Who receives {item}?" (party member by
  name, or "First available hero") whenever the chosen bonus is equipment —
  threaded through as `pendingRegionBonusHeroSlot`, same passthrough chain
  `pendingRegionBonusId` already uses. Load Game's live in-battle fallback
  prompt gained the same follow-up.
- `BuildSystem.grantFreeCharge`/`freeChargesFor`/`consumeFreeCharge`/
  `wasFreePlaced` — free placement-charge bookkeeping, reused by a region's
  structure/trap bonus (now player-placed via ordinary Build mode, 2
  charges instead of 1 auto-placed structure).

Changed:
- `grantEquipmentOrSellForGold` narrows to exactly the chosen hero when a
  recipient was picked, selling for gold instead of redirecting elsewhere
  if that hero no longer has room by battle-start.
- 12 equipment items referenced by region bonuses (`data/equipment.ts`)
  repriced so their sell value lands between their region's two gold
  tiers, instead of always being worth less than either (every
  common/uncommon non-attunement item in the catalog was priced 6-16g
  against 15-65g gold tiers). Also sold at the new price in the general
  Armory/GearShop.
- `tryBuild`/`refundStructure` are charge-aware: a free-charge placement
  skips the gold cost, and does NOT refund gold if later removed.

See `DECISIONS.md` D-250 for the full design (including the two forks
confirmed with Kevin) and `KNOWN_ISSUES.md` KI-198 for the playtest
checklist. Batch F's item 18 (real autosave) confirmed deferred to its own
session, not part of this pass.

## [Unreleased] — 0.2.0-dev — Batch F: "Save Party" dropped from the pause menu for a campaign battle (D-249)

Continuing from D-248 (same day). Closes the last open half of item 17
(Batch B/D-238 kept both save buttons specifically pending this).

Changed:
- `PauseMenuScene` no longer shows "Save Party" during a campaign battle —
  a campaign's party build already autosaves via `CompanionRosterSystem` at
  Start Battle (Party Creation Overhaul Plan 3.1), so a mid-battle re-save
  of the same pre-battle build was redundant there. "Save Game" (which
  saves and exits, and is the only path that produces a `Load Game`-
  resumable slot) is untouched. Free Play/Co-op keep "Save Party" exactly
  as before — they have no equivalent autosave.

Added:
- `BattleScene.isCampaignBattle()` — a small new accessor, deliberately
  separate from `canSaveParty()` (which still gates "Save Game").

See `DECISIONS.md` D-249 for the full reasoning and `KNOWN_ISSUES.md`
KI-197 for the playtest checklist.

## [Unreleased] — 0.2.0-dev — Batch E: region-bonus dominance fix + "Choose a Bonus" moved before Character Creation/the Armory (D-248)

Continuing from D-247 (same day). With no new Kevin playtest confirmations
yet on the just-shipped economy arc, Batch E of the 2026-09-09 19-item
playtest list was next per `PHASE_HANDOFF.md`'s own standing instructions.

Fixed:
- `RegionBonusSystem.drawRegionBonusChoices` could draw both gold tiers (or
  both equipment, or both structure options) into the same 3-choice offer,
  leaving no real choice between them (the bigger one always won). Rewrote
  as a two-pass draw: one option per category first (guaranteed for the
  real 3-category/`count=3` call), then a fallback pass only reached if
  `count` exceeds the category count.

Added:
- `RegionBonusChoiceScene` (new, registered in `main.ts`) — the "Choose a
  Bonus" pick now happens BEFORE Character Creation (Chapter 1) or the
  between-missions Armory (Chapter 2+), instead of after the battle has
  already started. Reached from all three campaign-entry routing sites
  (`CampaignSelectScene`, `UnlockMissionPartyScene`'s two sites), which now
  consolidate their duplicated `getPcBuild` Armory-vs-Character-Creation
  ternary into this one scene. No-ops straight through for the Prologue and
  Nameless Throne capstone (neither has a bonus pool).

Changed:
- The chosen bonus's id now threads through as `pendingRegionBonusId`
  (`CampaignArmoryScene` → `CharacterCreationScene` → `BattleScene`, same
  passthrough `campaignId`/`chapterIndex` already use).
  `BattleScene.showRegionBonusChoiceIfAny` applies a pending pick directly
  (no second prompt), falling back to the original live draw-and-ask only
  for `LoadGameScene`'s direct-resume path (which bypasses the new scene).
  Granting timing is unchanged — gold/equipment/structure still apply at
  chapter-start exactly as before; only the ASKING moved earlier.

See `DECISIONS.md` D-248 for the full design and `KNOWN_ISSUES.md` KI-196
for the playtest checklist.

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 6: gold-scarcity tuning pass, closes the entire economy redesign (D-247)

Kevin said "Plan 6 now," continuing straight from D-246. Plan 6 is normally
gated on a real playtest of Plans 1-5 first (none of KI-186 through KI-194
had been confirmed yet) — asked Kevin directly, and he chose to have a
reasoned first-pass tune done anyway rather than wait.

Changed:
- `campaignGoldMultiplier` (`data/difficulty.ts`) roughly halved at every
  difficulty tier, preserving the original relative shape: easy 1.25→0.65,
  normal 1.0→0.5, hard 0.75→0.4, nightmare 0.5→0.25. Normal is no longer
  pinned to an unscaled 1.0x baseline (a deliberate D-242 convention,
  broken here with Kevin's explicit confirmation) — a numeric analysis
  found the old numbers likely let a Normal-difficulty player afford most
  of the rare/veryRare gear ladder well before the campaign's 24 missions
  end, since every rarity tier unlocks by level 13 at the latest — reached
  by ~chapter 3 of the very first region.

`startingCampaignGold` and `data/regionBonuses.ts`'s gold-bonus amounts
were left untouched — both already flow through the same (now-halved)
multiplier, so the fix lives in the multiplier alone.

See `DECISIONS.md` D-247 for the full analysis and `KNOWN_ISSUES.md`
KI-195 for the playtest checklist. **This closes
`CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` in its entirety** — all 6 plans shipped.

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 5: Gear Points retired, campaign PC gear no longer player-editable in Character Creation (D-246)

Kevin said "Plan 5 now," continuing straight from D-245 — this closes the
entire gameplay-changing arc of the economy redesign (only Plan 6, a
tuning pass, remains).

Added:
- `defaultStartingGearForClass(classId)` (`data/characterCreation.ts`) — a
  fixed, non-editable starting kit for a brand-new campaign PC, one per
  class, matching that class's own established companion's kit exactly
  (confirmed with Kevin: no player choice, same as a companion's floor
  loadout).

Changed:
- A campaign PC's "Gear" button in Character Creation is now inert, fresh
  or returning — the same "no free pick" treatment a companion already
  got. A fresh PC's gear is the new fixed per-class kit (reacts live to a
  class change); a returning PC's real gear is whatever they've actually
  bought in the between-missions Armory (D-243), unchanged from before.
  The "Pool" button (claim unclaimed party-inventory items) is unaffected.
- Free Play's gear picker is completely unaffected — still free pick, no
  budget, every slot.

Removed:
- "Gear Points" entirely — `DifficultyDefinition.startingGearPoints`,
  ...
  shape. Nothing else in the codebase referenced any of these.

See `DECISIONS.md` D-246 for the full design (including the Chapter-1
starting-kit design fork resolved directly with Kevin) and
`KNOWN_ISSUES.md` KI-194 for the playtest checklist.

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 4: the in-battle Armory is removed for campaign battles (D-245)

Kevin said "Plan 4 of economy next," continuing straight from D-244. A
campaign battle no longer offers the in-battle "Gear (G)" shop at all —
gear is bought/sold only in the between-missions `CampaignArmoryScene`
(D-243) now. Free Play's in-battle Armory is completely unchanged.

Changed:
- `BattleScene`'s "Gear (G)" button/key are hidden and non-functional for
  any campaign battle (`this.campaignId` set) — pressing G, or the button
  (which no longer even renders), logs "Gear is managed between missions
  in the Armory during a campaign." instead of opening the shop.
- The top banner's safe-width math reclaims the space the Gear button used
  to occupy for a campaign battle, so it still never overlaps Build/End
  Turn.

Purely subtractive — no new files, no new persisted state. See
`DECISIONS.md` D-245 for full detail and `KNOWN_ISSUES.md` KI-193 for the
playtest checklist.

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 2: real gold sources feed the persistent pool (D-244)

Kevin said "Plan 2 next." Kill/wave gold and region-bonus gold now actually
credit the persistent campaign gold balance (D-242) at chapter-victory,
instead of only ever inflating the doomed-to-be-discarded per-battle
economy.

Added:
- `BattleScene`'s `campaignRewardGoldEarned` — a running GROSS total of
  this battle's kill/wave/region-bonus gold, credited to `CampaignGoldSystem`
  (scaled by `campaignGoldMultiplier`) at a real chapter-victory only, via
  a new pure `CampaignGoldSystem.creditScaledCampaignGold` function.

Fixed / changed from the original plan doc wording (see `DECISIONS.md`
D-244 for the full reasoning):
- Uses a GROSS earned counter instead of the victory screen's `goldEarned`
  (which nets against in-battle spending) — spending gold at the still-live
  in-battle Armory mid-battle no longer reduces your persistent credit.
- Region bonus gold now credits the persistent pool too, but only at
  chapter-victory (not immediately when picked) — the bonus choice
  re-offers on every chapter retry, so crediting immediately would let a
  lose-and-retry loop farm free permanent gold.

See `KNOWN_ISSUES.md` KI-192 for the playtest checklist.

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 3: the between-missions Armory scene (D-243)

Kevin said "Plan 3 sounds good to me." The first genuinely player-visible
piece of the economy redesign — a new scene where the party shops with
persistent campaign gold before the next mission.

Added:
- `src/game/scenes/CampaignArmoryScene.ts` — a new scene at the
  `CampaignSelectScene -> CharacterCreationScene` seam (and
  `UnlockMissionPartyScene`'s equivalent hand-off points), shown only once
  a campaign PC build exists (never before Chapter 1). Shows the whole
  party (PC + every active companion) shopping against the persistent
  `CampaignGoldSystem` balance, full rarity (matching the in-battle
  Armory's level-gated catalog), via a THIRD backend for the shared
  `gearPickerView.ts` component. Also adds a "Party Inventory" Sell action
  for a currently-unclaimed shared-pool item.
- `CompanionRosterSystem.companionPurchasedGear` (+ `getCompanionPurchasedGear`/
  `setCompanionPurchasedGearSlot`/`applyPurchasedGearOverrides`) — lets a
  companion's Armory purchases/sales survive a later Character Creation
  visit, layered on top of their authored/difficulty-trimmed baseline.
- `PartyInventorySystem.sellPartyInventoryEntry` — the pool-entry Sell
  primitive the Armory's list uses.

Fixed:
- `CharacterCreationScene`'s campaign PC gear used to silently drop any
  rare-or-better item on the next visit (its gear-slot picker only ever
  understood a common/uncommon lookup table) — a new `pinnedGearIds`
  mechanism on `SlotState` now preserves anything that table can't
  represent, so a rare+ Armory purchase survives untouched. General fix,
  not campaign-PC-specific — also closes the same latent gap for a Free
  Play `loadedParty` save carrying an unmappable gear id.

See `DECISIONS.md` D-243 and `KNOWN_ISSUES.md` KI-191 (a real playtest
checklist this time) for full detail, including the two design forks
confirmed with Kevin before building (full rarity now vs. later; skipping
this scene before Chapter 1).

## [Unreleased] — 0.2.0-dev — `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` Plan 1: `CampaignGoldSystem`, the new persistent campaign gold currency (D-242)

Kevin said "let's work on the economy plan now." First of the plan doc's 6
sub-plans — the persistent-gold system itself and its difficulty levers,
not yet wired into any scene's earning/spending path (Plans 2-4's job).

Added:
- `src/game/systems/CampaignGoldSystem.ts` — a persistent, shared campaign
  gold balance (`CampaignGoldState`), same pure/storage-agnostic shape as
  `CampaignLevelSystem`. `loadCampaignGold`/`saveCampaignGold` (own
  `CAMPAIGN_GOLD_STORAGE_KEY`), `earnCampaignGold`, `spendCampaignGold`
  (fails cleanly, unchanged state, when unaffordable), `canAffordCampaignGold`.
- `DifficultyDefinition.startingCampaignGold` (90/70/50/30 per tier) and
  `.campaignGoldMultiplier` (1.25/1/0.75/0.5) — campaign-mode-only, defined
  now so Plans 2/3 have a lever to read once they exist; not yet applied
  anywhere.

Changed:
- `CampaignSelectScene`'s "Reset Campaign Progress" button now also resets
  `CampaignGoldSystem` to its default, matching the existing `campaignLevel`
  reset it already performed.

No player-visible surface yet — no scene displays, earns, or spends
campaign gold. See `DECISIONS.md` D-242 and
`CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` (Plan 1 now marked DONE) for full
detail.

## [Unreleased] — 0.2.0-dev — Batch D DONE: gear-picker unification (items 3/5) via a new shared `gearPickerView.ts` component (D-241)

Kevin resumed Batch D after confirming `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md`'s
3 open questions. Closes items 3/5 of the 2026-09-09 playtest list.

Added:
- `src/game/scenes/gearPickerView.ts` — the shared gear-shopping rendering
  component (`GearPickerView`, `GearPickerBackend`/`GearPickerEconomy`,
  `goldEconomy`/`pointsEconomy`/`FREE_ECONOMY`) extracted from
  `GearShopScene.ts`. Both `GearShopScene` ("The Armory") and
  `CharacterCreationScene`'s gear picker now drive this one component
  instead of two separate hand-rolled implementations.

Changed:
- `GearShopScene.ts`: rewritten to a thin `GearPickerBackend` adapter over
  `BattleScene` — behavior unchanged from before this split.
- `CharacterCreationScene`'s gear picker: rebuilt to structurally match The
  Armory — a multi-hero sidebar (switch heroes without closing/reopening),
  Hands (Weapon/Shield) and Rings both consolidated with independent
  per-occupant Sell/Unequip buttons (item 5's fix), rarity/magic/hands
  category filters, a real compare strip. The campaign PC's Gear Points
  economy now shows unaffordable items greyed out ("Need N pts") instead of
  silently hiding them — a deliberate parity change with the Armory's gold
  pattern. Weapon-proficiency filtering now goes through the same shared
  `GearFilterSystem.applyCatalogFilters` call the Armory already used.
- Fixed a real gap: pressing Esc/Back while the gear picker was open used
  to jump straight to Main Menu — now guarded like every other overlay.

See `DECISIONS.md` D-241 and `KNOWN_ISSUES.md` KI-190 for full detail,
including the deliberate design calls (economy shapes, the one accepted
approximation, why Gear Points' UX changed).

## [Unreleased] — 0.2.0-dev — Playtest list item 1: campaign PC identity/ability-score lock reverses D-213 (D-240); Batch D paused, campaign economy redesign scoped instead

Kevin's 2026-09-09 playtest list, item 1 (confirmed directly with Kevin: it
reverses D-213, not just companions). Batch D's items 3/5 (gear-picker
unification) are PAUSED — mid-session Kevin redirected toward scoping a
much bigger campaign-economy redesign (persistent gold replacing Gear
Points, in-battle Armory removed for campaign battles) — see
`CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` (new document, scoping only).

Changed:
- `CharacterCreationScene`: the campaign PC's Class/Race/Background/
  ability-score controls now lock once a campaign persists a build
  (`identityLocked`/`abilityScoreLocked`), matching a companion — Gear,
  Spells, Level Plan, and Name stay editable (`gearLocked` stays false for
  the PC).
- Fixed a bug this surfaced: the Subclass picker had no lock guard at all
  (every other identity control did) — a locked companion of a
  level-1-subclass-choice class could freely change subclass before this
  fix.

Added:
- `CAMPAIGN_ECONOMY_REDESIGN_PLAN.md` — a 6-plan scoping document for the
  persistent-campaign-gold redesign Kevin proposed this session. Nothing
  in it is implemented yet.

See `DECISIONS.md` D-240 and `KNOWN_ISSUES.md` KI-189 for full detail.

## [Unreleased] — 0.2.0-dev — Playtest batch, Batch C: shrink-to-fit consolidation, renderAsiPrompt dynamic row heights, targeted text-overflow sweep (D-239)

Kevin's 2026-09-09 playtest list, continued (item 10's remainder — see
`PHASE_HANDOFF.md` for the full 8-batch plan; item 11 stays open, flagged
as multi-session in the plan itself).

Added:
- `uiTheme.shrinkFontToFit`: the shared "measure the real rendered size,
  shrink until it fits" primitive four call sites each used to hand-roll
  independently.
- `uiTheme.measureChoiceRowHeights`: the per-row dynamic height measurement
  `uiTheme.renderChoiceOverlay` already used, pulled out so
  `BattleScene.renderAsiPrompt` (previously a flat height constant) can use
  it too.

Changed:
- `BattleScene.renderAsiPrompt` (every ASI/feat/subclass/spell-pick screen):
  row height now sizes to its own tallest choice instead of a shared fixed
  constant — a long feat/spell description no longer overflows its box.
- Fixed a targeted set of real dynamic-content overflow risks in
  `BattleScene.ts`: the roster HUD's hero-name text, the boss/legendary
  on-token name banner, the shop/debug item-grid button label, the co-op
  Gold HUD line's partner-name variant, the multi-enemy-type wave-preview
  line, and three hero-name-bearing overlay titles (spellbook, spell-swap,
  and `renderAsiPrompt`'s own title).

See `DECISIONS.md` D-239 and `KNOWN_ISSUES.md` KI-188 for full detail,
including what was deliberately left alone and why.

## [Unreleased] — 0.2.0-dev — Playtest batch, Batch B: shared-overlay reskins, gear-compare deltas, Gear button label, dialogue-box hint/Skip move, pause-menu reorder (D-238)

Kevin's 2026-09-09 playtest list, continued (items 2, 4, 9, 10's root cause,
14, 17's reorder — see `PHASE_HANDOFF.md` for the full 8-batch plan).

Changed:
- `uiTheme.createOrnateButton`: its `sublabel` text now wraps to its
  button's own width instead of overflowing it unbounded — the single
  biggest lever on the "overlapping/spilling text" complaint, since this
  function is shared by ~20 scenes.
- `uiTheme.renderChoiceOverlay`/`openChoiceList`: reskinned from plain
  `add.rectangle()` boxes and `system-ui` text to the shared wood-panel/
  parchment theme (`createOrnateButton`, `FONT_DISPLAY`) — the one shared
  "pick one from a list" overlay that missed the earlier D-221/D-225 pass.
- `GearCompareSystem.formatGearDelta`/`previewGearSlotChange`: now reports
  damage-dice, saving-throw, movement, ability-score-setting, status-
  immunity, and charged-spell deltas, not just AC/attack bonus. The
  "nothing changed" fallback text is now "No change" (was "No AC/attack
  change").
- `CharacterCreationScene`'s Gear button always reads "Gear" now, instead
  of a preview of currently-equipped items.
- `dialogueBox.ts`: added a pulsing "Click anywhere to continue" hint;
  moved the Skip button from top-left to the bottom row (left-of-center).
- `PauseMenuScene`: reordered to Resume Battle, Controls, Settings, Save
  Party, Save Game, Load Game, Exit to Main Menu; "Save & Exit" renamed to
  "Save Game" (same behavior, per Kevin's own naming call this session).

See `DECISIONS.md` D-238 and `KNOWN_ISSUES.md` KI-187 for full detail and
Kevin's confirmation checklist.

## [Unreleased] — 0.2.0-dev — Playtest batch, Batch A: build-mode tile highlighting, correct build attribution, Emberford Ch2 unplayable bug (D-237)

Kevin's 2026-09-09 playtest list (19 items, planned as 8 batches — see
`PHASE_HANDOFF.md`). Batch A: the three confirmed bugs that were fully
independent of everything else.

Added:
- `BattleScene.showBuildableHighlights`: highlights every legal tile the
  instant build mode opens (or a different item is selected), instead of
  only reacting one tile at a time as the mouse hovers.
- `BattleScene.attributionHeroIdFor`: a placed structure is now attributed
  to whichever hero the player actually had selected before opening the
  build menu, falling back to the previous nearest-hero-to-tile behavior
  only when no hero was selected.
- `BattleScene.showChapterDialogue`: a defensive helper that destroys any
  still-open chapter dialogue before opening the next one, closing the
  structural gap behind Emberford Reach Chapter 2 becoming unclickable
  (an orphaned, invisible, full-screen dialogue overlay silently eating
  every board click while hotkeys kept working).

See `DECISIONS.md` D-237 and `KNOWN_ISSUES.md` KI-186 for full detail and
Kevin's confirmation checklist.
...
phase of Kevin's 2026-08-28 playtest batch — designed via an interactive
HTML mockup ("The Battlefield") before any code, same practice D-209 just
used. See `DECISIONS.md` D-210 for the full writeup.

Added/Changed:
- The battle screen's background, HUD buttons, roster panel, action-button
  row, shop/build catalog grid, and combat log all now share the wood/
  bronze/gilt fantasy theme (`uiTheme.ts`, D-123) that Main Menu,
  Compendium, Bestiary, and Character Creation already carry, instead of
  the original flat dark chrome.
- A new bold double bronze/gilt frame with corner diamonds now hugs the
  outer edge of the game board itself.
- Hero roster HP bars simplified from a three-tier green/gold/red gradient
  to a two-state green-until-critical-then-red (Kevin's own call).
- The combat log now sits on a real backing panel (bronze hairline rules,
  translucent dark fill) that grows/shrinks with the log's current line
  count, same as the text above it always has.
- The wave/phase banner now sits on a wood-panel/bronze chip sized to its
  own text.
- Every action-row button (Confirm/Cancel/Ability/Potion, Bonus Action/
  Action Surge/Class Action/Character Sheet, the hotkey bar) dropped its
  individual flat color and now matches the same neutral wood-panel style
  as the rest of the chrome, per Kevin's explicit ask — and several of
  these buttons have real hover/press feedback for the first time.
- **Deliberately untouched**: every tile/token/highlight color on the
  board itself, the "FANTASY TOWER DEFENSE" title, the debug-only Integrity/
  phase-history/enemy-count text, the Test Mode debug toolbar (still its
  own alarming red), and every full-screen modal overlay (ASI/subclass/
  spell-pick prompts, region-bonus choice, victory/defeat screens).

Tests: 1643 (unchanged — presentation-only). Typecheck clean, production
build succeeds (152 modules, unchanged).

## [Unreleased] — 0.2.0-dev — The Armory: gear-purchase UX redesign (D-209)

Phase 5 of Kevin's 2026-08-28 playtest batch — the "design-first, larger
builds" phase's first item, worked out over several rounds via an
interactive HTML mockup (not real code) before any implementation started.
See `DECISIONS.md` D-209 for the full writeup.

Added/Changed:
- The old in-battle "click an item, then click a hero" Gear grid is gone,
  replaced by a new full-screen scene (`GearShopScene`, launched the same
  way `PauseMenuScene`/`CharacterSheetScene` are): pick a hero, pick one of
  their 12 slots (10 gear + 2 potion, shown as a paperdoll), browse a
  catalog filtered to that slot.
- "Weapon"/"Shield" now read "Right hand"/"Left hand" in the shop, matching
  D-204's per-hero slot labels (which existed already but were never
  surfaced by the old Gear grid).
- Every catalog row has its own action button: Purchase on an empty slot;
  Compare → (a ~650ms delay) → Purchase on an occupied slot's other items;
  Sell → (the same delay) → Confirm Sell on the slot's own occupant — so a
  reflexive double-click can never spend or sell by accident.
- Buying into an occupied slot auto-sells the occupant as part of the same
  purchase (still no general inventory) — the trade-in shows on the price,
  and the Purchase button always states the real net cost, including
  "+Ng back" on the rare case a trade-in is worth more than the new item.
- **New economy rule**: selling gear/a potion — standalone, or as an
  auto-trade-in — pays half of cost, rounded down
  (`EconomySystem.sellValueForCost`), confirmed directly with Kevin. This
  replaces the old unequip action's 100%-refund behavior; every OTHER
  full-cost refund/award path (structure removal, a region-bonus/loot-drop
  reward with nowhere to go) is a reward or a build-mode action, not a
  player sale, and is unaffected.
- New pure module `systems/GearCompareSystem.ts` (`isItemEligibleForSlot`,
  `previewGearSlotChange`, `formatGearDelta`) replaces the old
  item-first `targetGearSlot`/`previewEquipDelta` — the new slot-first flow
  only ever needs "does this item fit the slot I already picked," not "which
  slot should this item go in."

Tests: 1643 (+16 — `sellValueForCost` cases in `tests/economy.test.ts`,
new `tests/gearCompareSystem.test.ts`). Typecheck clean, all pass,
...
`KNOWN_ISSUES.md` KI-159.

## [Unreleased] — 0.2.0-dev — Co-op economy: separate per-player gold pools (D-208)

Phase 4 of Kevin's 2026-08-28 playtest batch. See `DECISIONS.md` D-208 for
the full writeup.

Added/Changed:
- Gold is no longer a single flat number for the whole party — it's now one
  independent pool per economy owner. Solo play and every campaign always
  use one pool (`SOLO_ECONOMY_OWNER`), unchanged in behavior; a co-op
  session (Phase 12) gives each participant their own full starting-gold
  balance and independent spending, at Kevin's explicit choice ("the real
  co-op economy") over a shared-pool-with-attribution alternative.
- Kills are credited to whichever hero's own attack/ability caused them
  (the common case); a batch with no single attributable source (a trap
  kill, a bundled summon-kill sweep, wave-completion/time-bonus gold)
  splits evenly across every pool.
- Gear/potion purchases always charge the hero actually receiving the item;
  structure purchases charge whichever participant is clicking on that
  client; a Gold Thief steals from the specific hero it hits.
- Co-op's Gold HUD now shows both players' balances (`Gold: Ng | {partner}:
  Mg`); solo play's HUD is unchanged.

Tests: 1627 (+4). Typecheck clean, all pass, production build succeeds (150
modules, unchanged — no new file). No browser available in this
environment, and a real two-human confirmation needs Phase 12.3's own
result-broadcast follow-up (KI-063) before the two clients' boards actually
converge — see `KNOWN_ISSUES.md` KI-158.

## [Unreleased] — 0.2.0-dev — Phase 3's leftover HUD fix (D-207)

Closes the one item D-205 (Phase 3 of the 2026-08-28 playtest batch) left
unshipped — the Confirm/Cancel/Bonus Action/character-panel row reading as
"off center." See `DECISIONS.md` D-207 for the full writeup.

Fixed:
- The Confirm/Cancel/Ability/Potion/Bonus-Action/Class-Action/Character-Sheet
  button rows no longer reserve a fixed 86px gap under the roster for the
  combat log regardless of how much is actually in it — the gap now grows
  with the log's real content (starting small on Wave 1, before anything's
  logged), removing the large dead space Kevin's screenshot showed.
- Those same button rows now center whichever of their members are actually
  visible for the selected hero, instead of sitting at fixed left-to-right
  offsets sized for the whole row — a hero with only one or two of a row's
  buttons available no longer looks lopsided to one side.

Tests: 1623 (unchanged) — scene-only layout change, no new pure-system logic.
Typecheck clean, all tests pass, production build succeeds (150 modules,
unchanged). No browser available in this environment — see
`KNOWN_ISSUES.md` KI-157.

## [Unreleased] — 0.2.0-dev — Backgrounds (D-206)

Phase 2.5 of Kevin's 2026-08-28 playtest batch. See `DECISIONS.md` D-206 for
the full writeup.

Added/Changed:
- A new Background system — 4 real SRD 5.2.1 backgrounds (Acolyte, Criminal,
  Sage, Soldier) and 6 original ones (Siege Engineer, Ashfall Scout,
  Harborhand, Hedge-Warden, Ledger-Keeper, Ember-Marked), each granting 2
  skill proficiencies, a tool proficiency, a real ability-score-bonus choice,
  a single Origin Feat, and a starting weapon/gold nod.
- 4 new skills (Religion, Sleight of Hand, Arcana, History); skill
  proficiency is now background-aware as well as class-aware.
- New Background + ability-bonus picker in Character Creation; a new
  Compendium "Backgrounds" tab; all 12 companions assigned a fitting
  background.

Tests: 1623 (+16) — `tests/backgrounds.test.ts` and background-aware
coverage in `characterBuildSystem.test.ts`/`skillCheckSystem.test.ts`.
Typecheck clean, all tests pass, production build succeeds (150 modules,
+1 for `backgrounds.ts`). No browser available in this environment — see
`KNOWN_ISSUES.md` KI-156.

## [Unreleased] — 0.2.0-dev — Battle interaction upgrades (D-205)

Phase 3 of Kevin's 2026-08-28 playtest batch. See `DECISIONS.md` D-205 for
the full writeup.

Added/Changed:
- Move-then-attack: clicking an out-of-range enemy paths the hero toward it
  (using whatever movement is left this turn) and attacks in the same click,
  instead of rejecting the order outright. A ranged hero approaches only as
  far as needed to be in range, not all the way adjacent.
- Double-click-to-confirm: double-clicking a legal move tile commits the
  move immediately, no longer requiring a separate Confirm-button/Enter
  step.
- The per-hero customizable ability hotkey bar now has a real keyboard
  binding (Shift+1 through Shift+6, since plain digits already select a
  hero).
- Confirm/Cancel/Bonus Action are now rebindable in Settings' new "Controls"
  section (full replace — the new key takes over, XCOM2-style), with a
  conflict guard against the other rebindable keys and every other fixed
  hotkey.

Tests: 1607 (+14) — `HeroAISystem.planApproachForAttack` and a new
`keyBindings.test.ts`. Typecheck clean, all tests pass, production build
succeeds (149 modules, +1 for `KeyBindingSystem.ts`). No browser available
in this environment — see `KNOWN_ISSUES.md` KI-155.

## [Unreleased] — 0.2.0-dev — Character Creation depth (D-204)

Phase 2 of Kevin's 2026-08-28 playtest batch. See `DECISIONS.md` D-204 for
the full writeup.

Added/Changed:
- Standard Array now defaults to a per-class ability order (a Fighter's
  highest score lands on STR, a Wizard's on INT, etc.) instead of always the
  same flat STR/DEX/CON/INT/WIS/CHA order — applies to a fresh hero slot and
  to switching back into Standard Array from Point Buy; never re-applied to
  an already-customized hero on a later class change.
- Gear slot rename: "Weapon"/"Shield" → "Right hand"/"Left hand" (per-hero
  slot labels only — the shop/Compendium's item-type categories are
  unchanged). Picking a Two-Handed weapon into one hand slot in Character
  Creation now force-clears the other hand slot, and vice versa.
- New per-character library: any hero slot can "Save Character" (named,
  saved globally, survives across every campaign/party) and "Load
  Character" (pick a saved character, fully overwriting that slot). Backed
  by a new `CharacterLibrarySystem`, capped at 12 entries.

Tests: 1593 (+18) — `defaultAbilityOrderForClass`, `isTwoHandedWeapon`, and
a new `characterLibrarySystem.test.ts`. Typecheck clean, all tests pass,
production build succeeds (148 modules, +1 for `CharacterLibrarySystem.ts`).
No browser available in this environment — see `KNOWN_ISSUES.md` KI-154.

## [Unreleased] — 0.2.0-dev — Main Menu reorg + Campaign/Free Play New-or-Load forks (D-203)

Phase 1 of Kevin's 2026-08-28 playtest batch. See `DECISIONS.md` D-203 for
the full writeup.

Added/Changed:
- Main Menu reorganized: Campaign is now the primary "hero" button
  (replacing New Game); Free Play/Co-op/Party Creation form the secondary
  row (Party Creation consolidates the old duplicate "New Game"/"Build
  Party" buttons, which started the exact same screen); Compendium/
  Bestiary merge into one new "Knowledge Base" button; the standalone
  "Load Game" button is retired.
- New `ModeEntryScene` gives Campaign and Free Play each a "New X"/"Load X"
  fork before their existing screens ("New X" behaves exactly like the old
  direct buttons; "Load X" opens a filtered Load Game list).
- `LoadGameScene` gained an optional `filterMode` ("campaign" shows only
  campaign-linked saves, "freeplay" shows only the rest) — omitted
  entirely for the in-battle pause menu's own "Load Game" button, which is
  unaffected.
- New `KnowledgeBaseScene` — two ornate-styled buttons (Compendium,
  Bestiary).

Tests: unchanged at 1575 (pure scene/presentation wiring, not
unit-tested). Typecheck clean, all tests pass, production build succeeds
(147 modules, +2 for the two new scene files). No browser available in
this environment — see `KNOWN_ISSUES.md` KI-153.

## [Unreleased] — 0.2.0-dev — Two Known Issues bug fixes (D-201, D-202)

Picked from `KNOWN_ISSUES.md` — Kevin: "let's fix both here and now."

Added/Changed:
- **D-201**: a campaign party saved mid-battle (pause menu's "Save Party"/
  "Save & Exit") and reloaded via Load Game now correctly restores its
  companion identity/gear lock, point-buy ability scores, forced party
  size, and hidden difficulty picker — previously re-entered Character
  Creation fully free-pick with no campaign context at all. `SaveSlot`
  gains optional `campaignId`/`chapterIndex` fields.
- **D-202**: Character Creation now resumes a "plain" (non-campaign, non-
  Free-Play) draft — typed names, class/race, gear, everything — across a
  Back-to-Main-Menu-then-return round trip, instead of always resetting to
  defaults. No repro was ever obtained for this one; fixed on Kevin's own
  explicit go-ahead, flagged as unconfirmed against his original report.

Tests: 1575 (+5, new `saveSystem.test.ts` coverage for D-201;
`CharacterCreationScene` isn't unit-tested, so D-202 added none). Typecheck
clean, all tests pass, production build succeeds (145 modules, unchanged).
No browser available in this environment — see `KNOWN_ISSUES.md` KI-151/
KI-152.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 7: the level-progression reference screen (D-200)

The last remaining item on the Party Creation Overhaul roadmap — closes the
whole epic. A level-by-level (1-20) reference for what a class/subclass
gets at each level, including spell slots/cantrips/prepared counts for
casters. See D-200 for the full writeup.

Added/Changed:
- New pure `ClassProgressionSystem.ts` — assembles class/subclass feature
  lists with the existing spellcasting/spell-prep tables into a 1-20 table,
  no new game-balance numbers.
- Compendium's Classes detail view now shows a real per-level table
  (feature rows plus a condensed spell-slot/cantrip/prepared/spellbook
  line for casters) instead of a flat feature list. Subclasses tab is
  unchanged.
- Character Sheet gains a new "Progression" tab — the same table for the
  viewed hero's actual class+subclass, dimming levels not yet reached,
  with a hover tooltip for full feature text.

Tests: 1570 (+9, new `classProgressionSystem.test.ts`). Typecheck clean,
all tests pass, production build succeeds (145 modules, +1). No browser
available in this environment — needs Kevin's own pass — see
`KNOWN_ISSUES.md` KI-150.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 6: the level-up blueprint library (D-199)

The Character Creation "Plan Levels" wizard can now save a plan as a
named, reusable blueprint — usable by any future character of that class,
in any save or campaign, forever — and, per Kevin's own correction of
D-136, plan a caster's level-up-triggered spell swaps too. See D-199 for
the full writeup, including the two open design questions the plan doc
left unresolved (storage backend, decoupling the cadence setting).

Added/Changed:
- New global blueprint library (`BlueprintLibrarySystem.ts`, local-only
  storage) — create, edit, use as-is, and delete named blueprints per class.
- "Plan Levels" now opens a 3-way choice (Create New / Select Saved /
  No Blueprint) instead of jumping straight into the wizard; the Done
  screen gains a "Save as Blueprint" option alongside the unchanged
  this-hero-only "Save & Close".
- The Auto/Prompted/Fresh cadence setting moved OUT of the wizard into its
  own small per-hero pill next to "Plan Levels" — decoupled from "which
  plan," locked to Auto for an AI-controlled hero.
- A caster's level-up-triggered cantrip/prepared-spell swaps are now
  plannable in the wizard, resolved silently in "Auto" mode when fully
  planned, pre-highlighted in "Prompted" mode otherwise. The separate
  Long-Rest full-relist mechanic is untouched.

Tests: 1561 (+21 — a new `blueprintLibrarySystem.test.ts` plus more
`levelUpPlanSystem.test.ts` coverage). Typecheck clean, all tests pass,
production build succeeds (144 modules, +1 for the new system file). No
browser available in this environment — the largest UI addition on the
whole roadmap, needs Kevin's own dedicated playtest pass — see
`KNOWN_ISSUES.md` KI-149.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 5: AI-hero level-up defaults + Human/AI toggle clarity (D-198)

Kevin's own playtest complaint: an AI-controlled hero could still land a
real level-up choice popup (ASI/subclass/spell pick) with nobody there to
answer it, and the Human/AI toggle button's plain gray label made its
current state easy to miss. See D-198 for the full writeup, including the
two decisions confirmed directly with Kevin (a hard mode-lock rather than
just a smarter default, and inventing a simple choice rather than leaving
an AI hero permanently without one — a deliberate, narrow exception to
D-16x's "never invent a choice" rule).

Added/Changed:
- An AI-controlled hero's level-up mode is locked to "Auto" — Character
  Creation's Plan Levels wizard only offers "Auto-follow a blueprint" (plus
  Cancel) for an AI-controlled slot.
- An AI-controlled hero never sees an in-battle level-up popup: an
  unresolved choice (no blueprint entry) now applies a simple invented
  default (its class's primary ability +2, its first modeled subclass, the
  first eligible spell) instead of queuing a prompt nobody would answer —
  both for a real in-battle level-up and for the pre-battle Starting-Level
  fast-forward. Human/remote-controlled heroes are completely unaffected —
  still get the usual real popups.
- The Human/AI toggle button now has a genuinely distinct look per state
  (gilt border/brighter fill for AI, reusing the existing ornate-button
  "selected" treatment) and clearer text ("Hero N — AI-Controlled" /
  "Human-Controlled").

Tests: 1540 (+7, all new in `levelUpPlanSystem.test.ts`). Typecheck clean,
all tests pass, production build succeeds (143 modules, unchanged). No
browser available in this environment — needs Kevin's own playtest pass —
see `KNOWN_ISSUES.md` KI-148.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 2.3: shared party inventory pool (D-197)

An XCOM2-style shared gear pool between a campaign's active and benched
companions, per Kevin's own spec: benched companions' gear used to just
sit unreachable on them — now "Unequip All Benched Heroes" moves it into
a shared pool, any active hero can equip from it during party setup, and
mission start resolves it (claimed items become permanently the
claimer's, unclaimed items silently return to their original owner — no
item is ever lost or duplicated). See D-197 for the full writeup.

Added/Changed:
- New "Unequip All Benched Heroes" button (Companions screen), two-click
  confirm, plus a "Party Inventory: N items" count label.
- New "Pool" button next to Gear for every active hero in Character
  Creation (campaign mode) — draw a pooled item into any gear slot, same
  two-level picker shape as the existing Gear button.
- A reactivated companion whose gear was pooled no longer shows it as
  still equipped while it's sitting claimable in the pool.
- Campaign Select now forwards the selected Difficulty to the Companions
  screen, so unequipping accounts for a companion's actual
  difficulty-trimmed kit rather than assuming Normal.

Tests: 1533 (+22 — a new `partyInventorySystem.test.ts` plus more
`companionRosterSystem.test.ts` coverage). Typecheck clean, all tests
pass, production build succeeds (143 modules, +1 for the new system
file). No browser available in this environment — this is a multi-step
flow that needs Kevin's own playtest pass — see `KNOWN_ISSUES.md` KI-147.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 4: hero stat preview shows AC instead of ATK/Range (D-196)

Character Creation's per-hero stats line read `HP {n}  ATK {n}\nRange
{n}  Move {n}` — Kevin's own note: "Attack means nothing now that we've
switched to actual DnD character sheet based stats... AC should replace
those 2 useless stats." Both were already dead: ATK never factored in an
equipped weapon, and Range was a fixed melee/ranged constant off the
class, never the real weapon-aware range used everywhere else in the
game. See D-196 for the full writeup.

Added/Changed:
- The stats line now reads `HP {n}  AC {n}\nMove {n}`, computed off a real
  scratch `Hero` fast-forwarded to the chosen Starting Level under that
  hero's own level-up plan — the AC number genuinely reflects equipped
  gear, subclass bonuses, and ASI-granted feats (e.g. Defense fighting
  style), not just a level-1 guess.

Tests: 1511 (unchanged — pure UI wiring reusing already-tested
`LevelUpPlanSystem.simulateHeroForPlanning` and `Hero.armorClass`).
Typecheck clean, all tests pass, production build succeeds (142 modules,
unchanged). No browser available in this environment — the stats line's
layout and whether the AC number matches the in-battle HUD need Kevin's
own playtest pass — see `KNOWN_ISSUES.md` KI-146.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 3: campaign party/character persistence + Reset Campaign Progress (D-195)

The overhaul roadmap's own "biggest structural item": campaigns previously
had no cross-chapter continuity at all — every visit rebuilt the PC and
companions from scratch. Closed the ENTIRE plan (3.3, 3.1, 3.2, 3.5, 3.6,
3.7, and 3.4) in one session. Along the way, resolved a real architecture
question with Kevin directly: the game's companion roster/campaign
progress/world flags are (correctly, by design) shared across all 7
campaign entries as one continuous playthrough, not isolated per campaign —
so persisted builds live in that same shared storage rather than a new
per-campaign key, and a new "Reset Campaign Progress" action gives a real
clean-slate option instead. See D-195 for the full writeup, including why
per-campaign isolation was rejected.

Added/Changed:
- Once a campaign has been fully cleared at least once, its companions'
  ability scores unlock (class/race stay fixed to their identity) — a real
  reward for finishing the story, not available before then.
- A companion's or the PC's gear/spells/level-plan/name now actually
  persists between missions — editing them and clicking Start Battle
  commits the change, and it's still there on the next chapter visit.
- Once a PC's build has been committed once, their Class/Race/ability
  scores lock permanently for that playthrough (matching companions'
  existing lock) — gear/spells/level-plan/name stay fully editable.
- Companion ability scores (Standard Array dropdown, Point Buy +/-) are now
  actually locked in campaign mode — previously only Class/Race/Gear were.
- Difficulty is now chosen on Campaign Select, not in Character Creation —
  a new control there replaces Character Creation's own Difficulty button,
  which is hidden in campaign mode.
- The Party Size and "Save New Party"/"Update Saved Party" buttons are now
  fully hidden (not just disabled) in campaign-mode Character Creation —
  both are redundant now that campaign party state persists automatically.
- New: "Reset Campaign Progress" (Campaign Select, next to "Companions") —
  a two-click-confirm action that wipes the shared companion roster/
  persisted builds, campaign progress, and world flags back to defaults,
  for starting a genuinely new playthrough. Free Play saves are untouched.

Tests: 1511 (+9). Typecheck clean, all tests pass, production build
succeeds (142 modules, unchanged). No browser available in this
environment — the relocated Difficulty/Reset control row, the two-click
confirm's feel, and the full chapter-to-chapter persistence flow all need
Kevin's own playtest pass — see `KNOWN_ISSUES.md` KI-145.

## [Unreleased] — 0.2.0-dev — Campaign-mode gear economy: fixed difficulty-scaled companion kits + PC point-buy (D-194)

Kevin, right after seeing D-193's free-pick-everything gear system: in
campaign mode specifically, companions shouldn't be freely editable at
all (fixed kit, scaled down on harder difficulty, always keeping weapon +
class implement), and the PC should choose through a real point-buy
economy instead of picking for free. Free Play/manual Create Party is
unaffected. See D-194 for the full writeup.

Added/Changed:
- Campaign difficulty now sets a "Gear Points" budget for the PC's
  starting-gear picker (Easy 12, Normal 9, Hard 6, Nightmare 4 — first-
  pass numbers) — every item costs 1 (common) or 2 (uncommon) points, the
  picker shows items you can't afford as simply not there, and Start/Save
  are blocked if a later difficulty change pushes you over budget.
- Campaign companions' Gear button is now locked (matches Class/Race) —
  their kit is fixed, derived from their authored baseline, and scales
  with difficulty: full kit on Easy/Normal, chest armor dropped on Hard,
  chest and shield both dropped on Nightmare. Weapon and a caster's
  spellcasting focus/holy symbol/etc. always survive, at every difficulty.

Tests: 1502 (+11). Typecheck clean, all tests pass, production build
succeeds. No browser available in this environment; the point-buy UI's
feel, the companion-lock behavior, and whether these first-pass budget
numbers feel right in play all need Kevin's own playtest pass — see
`KNOWN_ISSUES.md` KI-144.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 2 (2.1 + 2.2): real multi-slot (all 10) starting gear + real per-companion kits (D-193)

Kevin: "Now do plan 2." Character Creation's single starting-gear pick
became a real per-slot loadout, and all 12 campaign companions got real,
authored starting kits instead of a generic flavor-item pool. 2.1 shipped
first as a 3-slot picker (Weapon/Armor/a class-appropriate Shield-or-Focus
third slot); Kevin then asked about the other 7 slots and picked full
expansion — every one of the 10 real gear slots is now an independent free
pick at creation, no class gating on any of them. 2.3 (party inventory) is
explicitly deferred — no meaningful persistence target exists for it until
Plan 3.1 is built. See D-193 for the full writeup.

Added/Changed:
- The Gear button now opens a real 10-row menu (Weapon, Shield, Head,
  Chest, Legs, Back, Ring 1, Ring 2, Amulet, Footwear) instead of a single
  flat pick list; each slot independently offers "None" plus every
  common/uncommon item of that slot type, open to every class.
- 4 new common-rarity spellcasting-focus items (Holy Symbol, Arcane Focus,
  Druidic Totem, Component Pouch), reusing the existing "amulet" gear slot.
- All 12 campaign companions now have a real weapon + chest armor + third-
  slot kit matching their class (e.g. Hollis Vane: Longsword/Chain Shirt/
  Shield; Isolde Varnhall: Dagger/Padded Armor/Arcane Focus), replacing the
  old single generic flavor-item pick; their other 7 slots start empty,
  fillable later via the in-battle Shop. Mira Quill (Monk) deliberately
  gets no chest armor, preserving her unarmored-defense AC formula.
- `CharacterBuild.startingEquipmentId`/`HeroDefinition.startingEquipmentId`
  are now legacy-only (kept for a pre-Plan-2 save's read-time fallback, not
  written by any new build) — replaced by `startingGearIds`, a per-slot map.

Tests: 1491 (+6 net). Typecheck clean, all tests pass, production build
succeeds. No browser available in this environment; the new UI and every
AC/attack-damage number it affects need Kevin's own playtest pass — see
`KNOWN_ISSUES.md` KI-143.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 1: ability-score assignment UX (D-192)

Kevin: "Let's build plan 1 now." Standard Array's click-to-cycle control
became a per-value dropdown with auto-swap; the Standard-Array-vs-Point-Buy
method choice moved from one scene-wide button to a small per-hero pill.
Point Buy's own controls are unchanged, per Kevin's explicit steer. See
D-192 for the full writeup.

Added/Changed:
- Clicking a Standard Array ability now opens a small dropdown offering the
  6 standard values (15/14/13/12/10/8) plus "—" (unset) instead of cycling
  through them one click at a time. Picking a value already assigned to a
  different ability swaps the two abilities' values.
- Each hero now has their own Standard Array/Point Buy method pill instead
  of sharing one party-wide toggle — one hero can use Point Buy while
  another uses Standard Array.
- Start Battle/Save Party are now blocked if a Standard Array hero has any
  ability left unassigned, with a status message naming the hero.
- `StandardArrayAllocator` (`CharacterBuildSystem.ts`) gained `assign()`/
  `valueFor()`/`isComplete()`; `cycle()` is removed. Its constructor and
  `allocatorFromScores` are unchanged, so no other call site needed edits.

Tests: 1485 (+3, `StandardArrayAllocator`'s suite rewritten against the new
`assign()` semantics). Typecheck clean, production build succeeds, `npm run
dev` serves HTTP 200. No browser available in this environment; needs
Kevin's own playtest pass — see `KNOWN_ISSUES.md` KI-142.

## [Unreleased] — 0.2.0-dev — Party Creation Overhaul Plan 8: ornate/parchment theme for Character Creation (D-191)

Kevin's own steer from the D-190 session: apply the D-123 ornate/parchment
theme to `CharacterCreationScene` right after Plan 0, before the plan's
other mini-plans build brand-new UI in the same scene. See D-191 for the
full writeup.

Added/Changed:
- Every hand-rolled button/panel in Character Creation (Human/AI toggle,
  Class/Race/Gear/Subclass/Starting Level/Plan Levels/Spells pickers, both
  ability-score control sets, Party Size/Difficulty/Ability Score Method/
  Team Level, Start Battle/Save Party) now uses the same carved-plaque
  ornate-button component Compendium/Bestiary/Main Menu already use, with
  real hover/click feedback instead of a flat rectangle.
- The scene's flat dark background is now the shared wood/stone
  `drawScreenBackdrop`; each hero column's plain dark panel is now a
  parchment panel, matching the rest of the game's fantasy chrome.
- Row heights and spacing grew slightly throughout the column to fit the
  taller ornate controls — no data/logic change, purely layout numbers.

Zero data/logic changes — every choice, validation rule, and computed
value is unchanged; this is a presentation-only pass. No browser available
in this environment; needs Kevin's own playtest pass — see `KNOWN_ISSUES.md`
KI-141 for the specific things to check.

## [Unreleased] — 0.2.0-dev — Party & Character Creation overhaul: roadmap plan + "Plan 0" bug fixes (D-190)

Kevin's first real playtest pass of Character Creation/campaign party
creation surfaced a large batch of notes. Result: a new roadmap doc,
`PARTY_CREATION_OVERHAUL_PLAN.md` (8 independent mini-plans), plus the
first mini-plan (bug fixes) shipped the same session. See D-190 for the
full writeup.

Added/Changed:
- The shared choice-picker overlay (`uiTheme.ts`'s `renderChoiceOverlay`,
  used by every Class/Race/Gear/Subclass button) now sizes each row to its
  own real wrapped-text height instead of a fixed constant — a long
  description no longer runs into the next row.
- Character Creation's Class/Race row labels now get the same overflow
  protection the Subclass row already had; the Race row also starts at a
  larger, easier-to-read font size.
- Subclass row wording shortened across all three of its display variants
  — long enough before that it could still overrun even at the smallest
  allowed font size.
- Point Buy's "Points Left" readout is now its own dedicated row instead
  of being appended onto (and pushing off-screen) the STR row's label.

Not fixed this session (needs a fresh repro from Kevin): hero name fields
reportedly moving/losing typed text across option screens — see
`KNOWN_ISSUES.md`'s "Open bugs" section.
...
masterpiece" to close out the 6-region campaign epic. The last remaining
piece of `CAMPAIGN_STORY_DESIGN.md` §5, flagged by every prior item-13
handoff since D-182. See D-188 for the full design/build writeup.

Added/Changed:
- **A new finale campaign, "The Nameless Throne"**, unlocks on Campaign
  Select once all 6 story regions are completed — a single climactic
  battle on a new, hand-authored map, culminating in one of two
  legendary bosses (Ashen Sovereign / The Hollow Empress, both statted
  since D-131 and unused until now).
- **Which boss and map dressing you face is decided by choices you already
  made** across the 6 regions — the 5 "Finish or Spare?" miniboss choices
  plus Sorrel Thane's fate arc, no new prompts required. Sparing/mercy
  leans toward the Ashen Sovereign ending; finishing/expedience leans
  toward The Hollow Empress. A real closing story beat plays before
  Victory: the Ashen ending has you name every companion still on your
  roster out loud; the Hollow ending has you reach for a name that isn't
  there anymore.
- Six new enemies (Ember Thane/Cinder Adept/Ashbound Honor Guard for the
  Ashen garrison; Drowned Thane/Hollow Caller/Drowned Honor Captain for
  the Hollow garrison) — reskins of existing stat blocks, no new balance
  work.
- New `systems/NamelessThroneSystem.ts` (pure, tested) and
  `data/namelessThroneMap.ts`.

Tests: 1444 → **1466**. Typecheck clean, production build succeeds (141
modules, +2 for the new map and system files), `npm run dev` serves HTTP
200. No browser available in this environment — see **D-188** in
`DECISIONS.md` and **KI-138** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — "A companion's own unlock mission must include them" (D-187)

Kevin's pick off item 13's remaining list: D-183's own last remaining
deferred item, the "required party member" rule. See D-187 for the full
clarification/rationale (the rule turned out to mean the newcomer fights
alongside you IN the unlock mission, not a later-content gate).

Added/Changed:
- **A new "Prepare the Mission" screen** appears before Character Creation
  whenever starting a battle that would recruit a new companion (a Pool A
  side mission, or a Pool B region's own Chapter 1 before that region's
  companion is recruited). Your own hero and the companion being unlocked
  are locked into the party; you freely choose the other 2 of the 4 slots
  from anyone already recruited (defaults to your current active roster).
- New `systems/UnlockMissionSystem.ts` (pure, tested) and
  `scenes/UnlockMissionPartyScene.ts`.
- `CharacterCreationScene` gained a `requiredCompanionIds` field so slots
  2-4 can be filled from an exact, explicit set of companions instead of
  always the active roster — every non-unlock-mission entry path
  (Free Play, Co-op, a loaded save, a normal chapter) is unaffected.
- No changes needed to the recruit-on-victory logic itself (D-183/D-186) —
  it only cares whether the campaign completed, not how the party for it
  was built.

Tests: 1434 → **1444**. Typecheck clean, production build succeeds (139
modules, +2 for the new system and scene), `npm run dev` serves HTTP 200.
No browser available in this environment — see **D-187** in `DECISIONS.md`
and **KI-137** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Pool A companion side-quest missions (D-186)

Kevin's pick off item 13's remaining slice list: D-183's own deferred
"side-quest missions" for the 3 locked Pool A companions. See D-186 for
the full rationale.

Added/Changed:
- **Every Pool A companion (Brand Ashcairn, Wren Calloway, Perrin Holt,
  Mira Quill, Cass Ferrow, Ellery Vance) now has its own unlock mission** —
  a fixed, flat, 3-wave battle reachable any time from Companion Roster's
  own locked card. Winning it recruits that companion onto the bench
  (never force-active).
- **Companion Roster's locked Pool A cards are now clickable**, launching
  the mission through the same Character Creation flow every other battle
  uses; the companion's name/class stay hidden until the mission is won.
- New `SIDE_MISSIONS` array (`data/campaigns.ts`) — deliberately kept out
  of `CAMPAIGNS` so these never show up as Campaign Select region cards.
  Each mission reuses an existing region map and an existing regular-tier
  enemy (never a miniboss/boss) — no new maps or enemies authored.
- New `CompanionDefinition.sideMissionId` field and `BattleScene.
  maybeUnlockSideMissionCompanion`, mirroring the existing Pool B home-
  region unlock exactly.

Tests: 1427 → **1434**. Typecheck clean, production build succeeds (137
modules — no new files). `npm run dev` serves HTTP 200. No browser
available in this environment — see **D-186** in `DECISIONS.md` and
**KI-136** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Sorrel Thane's fate arc: Redeemed / Marked / Lost (D-185)
...
flagged and deferred. See D-185 for the full rationale.

Added/Changed:
- **A "Sorrel Thane" choice appears at the start of Drowning Vale Chapters
  1-3** — a "support" or "press" pick each time, tracked toward one of
  three outcomes revealed at Chapter 4: **Redeemed**, **Marked**, or
  **Lost**. Redeemed/Marked stay flavor-only this pass (a combat-log line
  and a status tag, no stat/gear bonus).
- **Lost is permanent** — Sorrel is removed from the companion roster
  entirely (reuses the already-built `CompanionRosterSystem.loseCompanion`)
  and reappears as a new named threat, **Sorrel Thane, Lost**, taking
  Saltmere Shallows Chapter 1's returning-miniboss slot outright, ahead of
  any spared miniboss.
- New `systems/SorrelFateSystem.ts` (pure, tested) and a new
  `corrupted-sorrel` enemy (`data/enemies.ts`, stat block cloned from
  `tide-wretch`).
- No changes needed to `CompanionRosterSystem`/`CompanionRosterScene` —
  both already had the "Lost" mechanism and its own status rendering built
  and unwired.

Tests: 1412 → **1427**. Typecheck clean, production build succeeds (137
modules, +1 for the new system), `npm run dev` serves HTTP 200. No browser
available in this environment — see **D-185** in `DECISIONS.md` and
**KI-135** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — The Proving Ground: prologue mission gates the six regions (D-184)

Kevin's pick off item 13's remaining slice list: the "forced starting
mission" gate D-183 deferred. Rather than picking one of the 6 existing
regions to serve as the gate, Kevin asked for a brand-new one-time
mission instead — see D-184 for the full rationale.

Added/Changed:
- **New "The Proving Ground" prologue mission** — a small, theme-neutral
  map (`data/prologueMap.ts`) and 3 short low-difficulty waves, added as
  the first entry in `CAMPAIGNS` (`PROLOGUE_CAMPAIGN_ID`). Finale enemy is
  the existing `brute` minion, reused as a placeholder "boss" rather than
  authoring a new one.
- **All 6 story regions now stay locked** on Campaign Select (dimmed card,
  no click, "Complete The Proving Ground to unlock" hint) until the
  ...
  language (KI-130) rather than inventing a new look.
- No changes needed in `BattleScene`/`CharacterCreationScene`/Free Play —
  all three already treat a flat `campaignId` generically.

Tests: 1409 → **1412**. Typecheck clean, production build succeeds (136
modules, +1 for the new map file), `npm run dev` serves HTTP 200. No
browser available in this environment — see **D-184** in `DECISIONS.md`
and **KI-134** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Companion roster & recruitment UI, Phase 1 (D-183)

Kevin's pick off item 13's remaining slice list: companion recruitment UI.
His own design materially extends `CAMPAIGN_STORY_DESIGN.md` §6 — see
D-183 for the full rationale and what's still deferred (side-quest
missions, the starting-mission gate, forced-companion missions, Sorrel
Thane's fate arc).

Added/Changed:
- **A brand-new campaign playthrough now seeds a random starting trio**
  from the 6 class-coverage companions (Brand/Wren/Perrin/Mira/Cass/
  Ellery) — the other 3 stay locked, meant for future side-quest content.
- **Each region's own "mirror" companion (Hollis/Fenna/Isolde/Tamsin/
  Dorian/Sorrel) now unlocks onto the bench** the first time that region's
  Chapter 1 is completed — reusing `CompanionRosterSystem` (D-118
  scaffolding) for the first time.
- **New "Companions" screen**, reachable from Campaign Select, showing all
  12 by status (Active/Benched/Locked) and letting the player freely swap
  the active 3 with anyone benched, any time outside battle.
- **Campaign-mode Character Creation now auto-fills slots 2-4** with the
  roster's 3 active companions (identity locked, equipment/spells/hotkeys/
  starting level stay fully editable) and locks party size to 4. Free Play
  and a plain "Create Party" run are completely unaffected.
- Removed the now-obsolete `startsInParty` field (the old fixed-starting-
  trio design); added `POOL_A_COMPANION_IDS`/`POOL_B_COMPANION_IDS`.
- New `systems/CompanionSeedSystem.ts` (pure random-draw-3 rule) and
  `scenes/CompanionRosterScene.ts`.

Tests: 1402 → **1409**. Typecheck clean, production build succeeds (135
modules, +4 for the new system and scene files), `npm run dev` serves HTTP
200. No browser available in this environment — see **D-183** in
`DECISIONS.md` and **KI-133** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Returning-miniboss mechanic (D-182)

Kevin's pick off item 13's remaining slice list: build
`CAMPAIGN_STORY_DESIGN.md` §4's returning-miniboss mechanic.

Fixed/Changed:
- **Defeating one of the 5 "home" minibosses** (Basalt Colossus/Emberford,
  Juggernaut/Causeway, Gravemaw/Cinderfall, The Husk/Drowning Vale,
  Bloodrage Warlord/Frostbound) in its own Chapter 1 finale now shows a
  mandatory "Finish or Spare?" choice before the battle can end. Kill gold/
  loot/Bestiary credit are unaffected either way — sparing is a persisted
  narrative flag, not a reward penalty.
- **Saltmere Shallows Chapter 1** now reads that flag back: it spawns
  whichever earlier-region miniboss was spared (earliest region wins if
  more than one was) instead of the nameless Tide-Wretch, with a one-time
  "washed ashore" flavor line in the combat log the first time it spawns.
  Falls back to the Tide-Wretch, unchanged, if nothing was spared.
- Wires up `WorldFlagSystem` (D-118 scaffolding) for the first time — it
  existed fully built and tested but connected to nothing until now.
- New `systems/ReturningMinibossSystem.ts` (pure priority/fallback logic
  and the wave-list swap). Reuses each miniboss's existing enemy id/stat
  block verbatim for its return — no new "corrupted variant" enemies, only
  new one-line spawn flavor text per miniboss.
- Known, deliberate scope cut: the design doc's Sorrel-Thane-Lost priority
  ...
  that needs a companion-fate data model that doesn't exist yet, a separate
  item-13 slice. See D-182.

Tests: 1391 → **1402**. Typecheck clean, production build succeeds (131
modules, +2 for the new system file). No browser available in this
environment — see **D-182** in `DECISIONS.md` and **KI-132** in
`KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Pre-region bonus-choice screen (D-181)

Kevin's pick off item 13's remaining slice list: build
`CAMPAIGN_STORY_DESIGN.md` §8's HOMM3-style bonus-choice screen.

Fixed/Changed:
- **Every real campaign now offers a "Choose a Bonus" screen** before each
  chapter's battle (right after the chapter intro, before the ASI/
  subclass/spell-pick fast-forward prompts): 3 options, randomly drawn
  from that region's own curated 6-option pool, covering all four §8
  categories (gold, XP, equipment, a free structure/trap). Gold/XP amounts
  escalate by region tier; equipment options are capped to modest common/
  uncommon items across every region since the screen can appear at a
  region's own level 1-5 Chapter 1.
- Two real gaps in §8's own design resolved this session: "XP" has no
  literal currency to grant in this game's fixed per-wave leveling model,
  so it's modeled as a flat permanent max-HP grant instead (new
  `Hero.grantBonusHealth`); and "once per region" doesn't map onto this
  engine's fully self-contained per-chapter battles, so the bonus choice
  re-offers every chapter instead of only once. Both are documented,
  deliberate first-pass calls — see D-181.
- New `data/regionBonuses.ts` (the 6 curated pools) and
  `systems/RegionBonusSystem.ts` (the random-draw-3 rule). No new
  equipment/structure content — every item/structure referenced already
  existed.

Tests: 1377 → **1391**. Typecheck clean, production build succeeds (129
modules, +2 for the two new files). No browser available in this
environment — see **D-181** in `DECISIONS.md` and **KI-131** in
`KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Migrated the other 4 regions to real 4-chapter campaigns (D-180)

Kevin's own direct ask: "continue item 13 with building the other 4
regions" — the four `CAMPAIGN_STORY_DESIGN.md` §3 regions D-179 didn't
touch (that session only had two pre-existing flat campaigns to migrate;
these four had none at all).

Fixed/Changed:
- **Shattered Causeway, Cinderfall Rift, The Drowning Vale, and Frostbound
  Hollow are now real, chaptered campaigns** (D-180) — none had a
  `CampaignDefinition` before this session (their maps, built in Phase 23/
  D-114, were Free-Play-only). Each gets the same 4-chapter shape D-179
  established: Ch1 (1-5) ends in the region's own miniboss (Juggernaut /
  Gravemaw / The Husk / Bloodrage Warlord), Ch2 (6-10) a lighter first
  encounter with the region's real finale boss (The Devourer / Warlord
  Korrath / Blightmother / Sundered King), Ch3 (11-15) remixes the
  established roster at higher counts, Ch4 (16-20) exactly reuses a new
  flat 6-wave finale. All eight enemies already existed — no new roster
  content needed. Four new region-flavored curated loot pools added,
  matching the existing Phase 22 pattern.
- `CampaignSelectScene`/`CharacterCreationScene`/`BattleScene`'s existing
  D-179 chapter wiring is fully generic — no scene code changed at all;
  adding the four campaigns to the `CAMPAIGNS` array was enough.
- `FreePlayScene`'s unlock gating for these 4 maps and their 8 associated
  bosses, previously always-unlocked (no campaign existed to gate them
  on), now gates on completing that region's campaign — same treatment
...
modules, unchanged). No browser available in this environment — see
**D-180** in `DECISIONS.md` and **KI-130** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — Removed "signature action"; migrated 2 campaigns to real 4-chapter regions (D-178, D-179)

Kevin's own direct ask: remove the "signature action" concept from every
hero in the game (every action should come straight from the D&D class the
hero has levels in), and migrate the campaign structure. Two independent
changes, planned via `EnterPlanMode` given the scope.

Fixed/Changed:
- **No more "signature action"** (D-178): every hero's basic-attack
  baseline (melee/ranged style + ability-score scaling) now comes from a
  fixed per-class table (`data/classes.ts`'s new `basicAttackStyle`, paired
  with the pre-existing `primaryAbility` field) instead of a player choice.
  Removed entirely: `abilityId` from `CharacterBuild`/`HeroDefinition`/
  `Hero`, the Character Creation "Choose Signature Action" step, the 4
  invented non-SRD abilities (Cleave/Piercing Shot/Taunting Slam/Frost
  Bolt), and `BattleScene`'s `aimingAbility` interaction path. A non-caster
  hero's only offensive action is now its real weapon Attack (already
  fully weapon-driven — Extra Attack, weapon mastery, Divine Smite,
  Hunter's Mark, equipment procs all already worked independent of the old
  field) plus its existing class bonus/class-action buttons; casters'
  full-spellbook "Cast a Spell (Q)" is completely unaffected.
  `firestore.rules` updated to match. See **KI-128** for the one known,
  pre-existing (not worsened) limitation this surfaced.
- **Emberford Reach and Saltmere Shallows are now real 4-chapter regions**
  (D-179), the first live use of D-118's `chapters` engine scaffolding.
  Ch1 (levels 1-5) ends in a miniboss, Ch2 (6-10) a lighter first
  encounter with the region's real boss, Ch3 (11-15) remixes the
  established roster at higher counts, Ch4 (16-20) exactly reuses the
  existing flat 6-wave finale. A new enemy, **Tide-Wretch** (miniboss
  tier, deliberately no lore), stands in for Saltmere's Ch1 per the
  design doc's own documented fallback for its not-yet-buildable
  "returning miniboss" mechanic. `CampaignSelectScene`/
  `CharacterCreationScene`/`BattleScene` all wired for real chapter-by-
  chapter play (chapter progress display, chapter selection, per-chapter
  completion tracking, chapter intro/outro dialogue plumbing). See
  **KI-129** for the two known, accepted scope limitations (no cross-
  chapter continuity yet; end screen doesn't route back to Chapter
  Select).

Tests: 1370 → **1376** (net — mechanical + logic cleanup from D-178 across
~26 files, new chapter-content coverage in `tests/campaigns.test.ts` and
`tests/enemyRoster.test.ts` from D-179). Typecheck clean, all 1376 tests
pass, production build succeeds (127 modules, unchanged). No browser
available in this environment — see **D-178**/**D-179** in `DECISIONS.md`
and **KI-128**/**KI-129** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — KI-098 item 13 begins: 12-companion catalogue, one per class (D-177)

Kevin's own call to start item 13 (the overworld campaign epic, previously
the last untouched item on the KI-098 list), picking the companion-roster
slice first. On seeing `CAMPAIGN_STORY_DESIGN.md` §6's original six
companions, he asked to extend the roster to cover all 12 playable
classes — a soft, BG3-style tour of every class across the campaign, even
though only 3 are ever active alongside the PC at once.

Fixed/Changed:
- **`data/companions.ts`'s `COMPANIONS` array is now 12 real, playable
  `CharacterBuild`s** (D-177), up from the empty D-118 scaffolding. The
  original six named companions (Hollis Vane, Fenna Duskwater, Isolde
  Varnhall, Tamsin Rourke, Dorian Wick, Sorrel Thane) each got a class
  matching their already-written personality (Fighter, Druid, Wizard,
  Paladin, Warlock, Ranger — none had one assigned before this session).
  Six new companions (Brand Ashcairn, Wren Calloway, Perrin Holt, Mira
  Quill, Cass Ferrow, Ellery Vance) cover the remaining classes (Barbarian,
  Bard, Cleric, Monk, Rogue, Sorcerer) as ordinary recruits with a one-line
  hook apiece — deliberately lighter-weight than the original six's
  region-boss "mirror" relationships, since only six regions/bosses exist
  for those six to pair with.
- Every build is genuinely valid: real race/class combinations, a
  class-appropriate ability-score priority order, a flavor-matched
  signature action/cantrip, and one starting common/uncommon item apiece
  (all 12 `STARTING_GEAR_IDS` entries used exactly once). Subclass is set
  only for the three level-1-choice classes (Cleric/Sorcerer/Warlock),
  exactly the way `CharacterCreationScene` sets one for a player-built
  hero — every other class picks its subclass through the normal
  in-battle level-up queue once that companion actually reaches the choice
  level.
- `CompanionRosterSystem.ts` needed zero changes — confirmed it was
  already generic over roster size before writing any data.
- `CAMPAIGN_STORY_DESIGN.md` §6/§9 updated to document the 12-companion
  roster; `CONTENT_SOURCES.md` gained the entry its own §9 had flagged
  ("shouldn't [get one] until it's actually implemented as data").

Tests: 1377 → **1384** (+7, `tests/companions.test.ts` rewritten from
"catalogue is empty" assertions to real shape checks). Typecheck clean,
all 1384 tests pass, production build succeeds (**127 modules,
unchanged**). No browser needed or possible yet — pure data, no scene/UI
consumer exists to check on-screen. See **D-177** in `DECISIONS.md` and
item 13 in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — KI-098 item 9: bigger maps (D-176) — closes the KI-098 build backlog

The last actual build item on KI-098 — and the one flagged repeatedly as
the riskiest, since two prior sessions (D-157/D-159) broke Main Menu and
Character Creation attempting a `Scale.RESIZE` cutover, and D-172 found a
third risk in raising the global canvas size. Neither ruled-out direction
was needed: went with a `BattleScene`-LOCAL dynamic per-map tile size,
mirroring a pattern `MapBuilderScene` already ships and has worked
reliably in for several phases.

Fixed/Changed:
- **Dynamic per-map tile size in `BattleScene`** (D-176): every one of
  `BattleScene.ts`'s ~68 `TILE_SIZE` usages turned out to be grid/world-
  space (tokens, highlights, VFX) with zero HUD/chrome usages — so this
  is a pure grid-rendering change, no HUD code touched. Extracted the
  shrink-to-fit math `MapBuilderScene.rebuildGridSystem()` already used
  inline into a new pure, unit-tested `computeFittedTileSize()` (in
  `GridSystem.ts`), had `BattleScene` compute its own available grid area
  (full canvas width; height minus a fixed 403px HUD reserve below the
  grid) and construct `GridSystem` with a computed tile size instead of
  the constant, then replaced all ~68 remaining `TILE_SIZE` references
  with `this.grid.tileSize`. Every existing shipped map still renders at
  the exact same 64px tile size as before — verified both by the math and
  by an explicit regression test.
- **`MAX_MAP_COLS`/`MAX_MAP_ROWS` raised from 20/9 to 32/14** (D-176,
  `MapBuilderSystem.ts`), re-derived against a chosen 40px minimum-
  tile-size legibility floor (flagged as a first-pass balance value, same
  status as `STARTING_GOLD`). `firestore.rules`' hand-unrolled
  `isValidTileRows` (rules have no loop construct) extended from 9 to 14
  rows to match, plus the `cols`/`rows` numeric bounds.
- This closes the entire KI-098 "build" backlog — only item 13 (the
  overworld campaign epic, Kevin's own explicit lowest priority) remains.

Tests: 1370 → **1377** (+7, new `tests/gridFitting.test.ts` covering
`computeFittedTileSize` including an explicit shipped-map-regression case).
Typecheck clean, production build succeeds (**127 modules, unchanged** —
the new function lives inside the existing `GridSystem.ts` module). No
browser available in this environment — this is a real rendering change
(every token/VFX size is now computed, not constant) that needs Kevin's
own look, folded into his eventual combined KI-098 playtest pass. See
**D-176** in `DECISIONS.md` and **KI-127** in `KNOWN_ISSUES.md`.

## [Unreleased] — 0.2.0-dev — KI-098 items 10-11/12: initiative + level cadence (D-174, D-175)

Continuing straight down the KI-098 backlog. Before building items 10-12
("ask Kevin's decision, then build"), checked the actual code behind each
one first — two of the three turned out to rest on assumptions that didn't
match reality, caught before any code was written.

Fixed/Changed:
- **Level cadence LOCKED to every wave, no XP-split toggle built** (D-174):
  items 11/12 assumed a per-hero XP/kill-crediting economy that doesn't
  exist anywhere in this project — leveling has always been uniform (every
  living hero levels together, automatically). Asked directly, Kevin drew
  a line the project hadn't drawn before: the in-battle 1-20 class-level
  track should advance every SINGLE wave (not per-kill, no split needed),
  while a genuinely separate overworld/campaign-only XP track (unlocking
  story-progress bonuses) is real but not yet designed and belongs to item
  13's epic instead. The entire code change is
  `ProgressionSystem.LEVEL_UP_WAVE_INTERVAL` 2 → 1 — existing tests already
  assert relative to the constant, so nothing else needed to change. Also
  resolves `SOURCE_OF_TRUTH.md` §9's long-OPEN "Level cadence" item. See
  **D-174** in `DECISIONS.md` and **KI-125** in `KNOWN_ISSUES.md`.
- **Per-group initiative for the enemy phase** (D-175): Kevin's answer to
  the granularity question was per-group. `WaveSystem.tickEnemyPhase` used
  to process every enemy in raw spawn order with no turn-order concept at
  all (the "boss already gets its own turn" framing this was asked against
  turned out to be inaccurate — corrected before building). Enemies are now
  grouped by TYPE (`EnemyDefinition.id`), each group's initiative rolled
  once per wave via the previously-built-but-never-used `InitiativeSystem`
  (Phase 13.5's "framework only" scaffold), and the enemy phase processes
  groups in that order every tick — `this.active` is sorted IN PLACE so a
  same-phase reinforcement/summon still gets to act the instant it spawns,
  exactly as before this change. Also resolves `SOURCE_OF_TRUTH.md` §9's
  "Initiative" item. See **D-175** in `DECISIONS.md` and **KI-126** in
  `KNOWN_ISSUES.md`.

Tests: 1367 → 1370 (+3 new `tests/waveInitiative.test.ts`; no new tests for
D-174, a single-constant change to already-tested arithmetic). Typecheck
clean, all 1370 tests pass, production build succeeds (**127 modules, +1**
— `InitiativeSystem.ts` is now a real production dependency for the first
time). No browser available in this environment — needs Kevin's own look,
folded into his eventual combined KI-098 playtest pass.

## [Unreleased] — 0.2.0-dev — KI-098 item 8: hero-side split movement (D-173)

Continuing straight down the KI-098 backlog per Kevin's standing
instruction — this session's item.

Fixed:
- **Hero-side split movement: move, act, then move again, same turn**
  (D-173): the last piece of the Enemy AI/Movement Redesign epic.
  `Hero.movementBudget()` used to be all-or-nothing — any move at all
  zeroed it for the rest of the turn (an explicit "MVP: one move per turn"
  rule). It now tracks real leftover tiles (`movementTilesUsedThisTurn`),
  and `Hero.moveTo(dest, tilesUsed?)` deducts only what a move actually
  cost instead of the whole allowance. The existing selection/
  range-highlight/click-to-move UI and the D-144 drag-and-drop move needed
  **no changes to their own logic at all** — they already re-derive
  everything from `hero.canMove()`/`hero.movementBudget()` live, so a hero
  moved partway, then acted, now automatically shows its real leftover
  move range the instant it's reselected. A Rogue's Cunning Action Dash
  also got a real math fix as a side effect — it now stacks a full extra
  speed on top of whatever's left, real SRD math, instead of just
  refreshing the move slot (which only ever gave the right answer when the
  whole move had already been spent). See **D-173** in `DECISIONS.md` and
  **KI-124** in `KNOWN_ISSUES.md`.

Tests: 1358 → 1367 (+9 new `tests/d173Features.test.ts`). Typecheck clean,
all 1367 tests pass, production build succeeds (126 modules, unchanged).
No browser available in this environment — needs Kevin's own look, folded
into his eventual combined KI-098 playtest pass.

This closes out the Enemy AI/Movement Redesign epic entirely (D-139
through D-146, plus this).

## [Unreleased] — 0.2.0-dev — KI-098 backlog, items 1-7 + real movement math (D-165 through D-172)
...
Fixed:
- **Compendium item descriptions moved to a hover tooltip** (D-165): every
  itemized tab (Classes, Subclasses, Races, Feats, Skills, Spells,
  Equipment, Potions, Buildings, Traps, Status Effects) now shows a
  compact, paginated list of one-line rows instead of every entry's full
  description sitting permanently on the page — hovering a row shows its
  full description via the same shared `tooltip.ts` controller Gear/Shop
  item buttons already use (D-158). Nested categories (a subclass's
  features, a race's traits, Buildings'/Traps' sub-groups) get a bold,
  non-hoverable group-label divider row. The Dialogue demo tab is
  unchanged. See **D-165** in `DECISIONS.md` and **KI-116** in
  `KNOWN_ISSUES.md`.
- **The Character Sheet's hotkey bar now fires in battle** (D-166): a new
  4th HUD row shows a clickable button for each filled hotkey slot while a
  hero is selected, firing the exact same resolver Q/R/F/T's own paths
  already use (`chooseSpell`/`onBonusActionButton`/`onClassActionButton`/
  `onAbilityButton`) — pinning something to a slot in the Character Sheet
  now actually does something in battle. A new shared
  `hotkeyDisplayLabel` in `HeroActionRegistry.ts` replaces the near-
  identical label logic the Character Sheet already had, so the two
  screens can't drift apart. Mouse-click only — see D-166's own writeup
  for why no new keyboard shortcut was added. See **D-166** in
  `DECISIONS.md` and **KI-117** in `KNOWN_ISSUES.md`.

- **Equip-flow UX rethink** (D-167): the always-visible hero roster strip
  (previously decorative) is now a real click/hover target — in equip mode
  it targets that hero directly (same equip/unequip logic, plus the same
  before/after AC/attack hover preview the board token already had),
  outside equip mode it's a second way to select a hero. The original
  board-token flow is completely unchanged. See **D-167** in
  `DECISIONS.md` and **KI-118** in `KNOWN_ISSUES.md`.

- **The Character Sheet can now cast spells/use actions directly** (D-168):
  the Spellbook tab's per-spell cards and the Stats tab's "Available Right
  Now" action lines are real click targets now, not read-only — clicking
  one closes the sheet and fires it on the resumed battle through two new
  public `BattleScene` methods, sharing the exact same resolution rule the
  hotkey bar (D-166) uses. See **D-168** in `DECISIONS.md` and **KI-119**
  in `KNOWN_ISSUES.md`.

- **Main Menu: added a "Build Party" entry point** (D-169): a new button in
  the "Continue Your Journey" row opens the same Character Creation screen
  "New Game" does — which already supports Save Party + Back without ever
  starting a battle — under an honestly-labeled entry instead of leaving
  that capability undiscoverable. See **D-169** in `DECISIONS.md` and
  **KI-120** in `KNOWN_ISSUES.md`.

- **Five more playable races** (D-170): Dragonborn, Gnome, Goliath, Orc,
  and Tiefling join the starter six (11 total), all verified against the
  real SRD 5.2.1 species chapter. Also caught and corrected a sourcing-
  attribution error in the original six — Half-Elf/Half-Orc are actually
  SRD 5.1 species (SRD 5.2.1 dropped both), and SRD 5.2.1 gives every
  species the same 30ft speed (the 25ft Dwarf/Halfling split is SRD 5.1's
  rule). No game data changed — attribution only; see **D-170** in
  `DECISIONS.md` and **KI-121** in `KNOWN_ISSUES.md`.

- **Class-based movement bonus** (D-171): Monk's Unarmored Movement (level
  2+) and Barbarian's Fast Movement (level 5+) — both already-named,
  previously inert SRD features — now grant a real +1 tile via a new
  derived `Hero.classMovementBonus()`. See **D-171** in `DECISIONS.md` and
  **KI-122** in `KNOWN_ISSUES.md`. ("Move speed should scale with map
  size" is a separate, still-open, genuinely ambiguous ask — Kevin was
  asked directly rather than guessed at; it didn't block this item.)

- **Real D&D movement speed: 1 tile = 5ft, exactly** (D-172): every
  movement-tile number in the game rescaled from an abstracted "1 tile ≈
  10ft" to real D&D math — standard speed 3→6 tiles, Dwarf/Halfling 2→5
  (their exact 25ft, not a blind double), Goliath finally gets its own
  35ft/7-tile tier instead of D-170's flattening, the Monk/Barbarian class
  bonus and every gear/potion movement bonus and the `slowed` status all
  rescaled to match, and all 63 enemies' `movementTiles` doubled to
  preserve existing hero-vs-enemy pacing exactly. Also surfaced (while
  planning "bigger maps," item 9, alongside this) a previously-undocumented
  regression risk in one of D-159's two documented map-size options —
  see **D-172** in `DECISIONS.md` and **KI-123**/updated item 9 in
  `KNOWN_ISSUES.md`. The actual map-size increase is deferred pending a
  different technical approach.

Tests: 1348 → 1358 (+4 `hotkeyDisplayLabel`, `tests/races.test.ts` updated
in place with no net count change, +6 new `tests/d171Features.test.ts`;
D-172 touched 10 more test files' hand-built maps/expected positions with
no net count change). Typecheck clean, all 1358 tests pass, production
build succeeds (126 modules, unchanged). No browser available in this
environment — needs Kevin's own look, folded into his eventual combined
KI-098 playtest pass.

## [Unreleased] — 0.2.0-dev — August 2026 playtest bug batch (D-160 through D-164)

Kevin's next playtest pass reported 8 items in one message. Investigated
with parallel Explore/Plan agents reading the actual current code before
writing anything — several were real recurrences of previously-"fixed"
bugs, one clarified into a specific root cause via a follow-up question,
and one (the level-by-level Character Creation planner) turned out to
already exist (D-133), needing only Kevin's own browser confirmation, not
new work. See **D-160** through **D-164** in `DECISIONS.md` for the
complete method, and **KI-111** through **KI-115** in `KNOWN_ISSUES.md`
for the re-confirmation checklists.

Fixed:
- **Character Creation** (D-160): the 4 hero-name DOM inputs no longer
  stay visible on top of every picker/wizard overlay; the Back button
  moved from a hand-rolled bottom-center control to the standard top-left
  slot 9 of 12 other scenes already use.
- **Main Menu** (D-161): Settings and Exit Game no longer overlap
  `drawScreenBackdrop`'s frame border (Settings was overlapping by 8px;
  Exit Game had only 4px of clearance).
- **Horizontal-squish mitigation** (D-162, unconfirmed): `MainMenuScene`
  now calls `this.scale.refresh()` on every entry as a best-effort fix for
  a reported post-battle rendering desync whose exact mechanism couldn't
  be confirmed by reading the code alone — flagged explicitly for Kevin to
  re-check, not claimed as definitely fixed.
- **No more silently-invented level-up choices** (D-163): raising a hero's
  Starting Level/Team Level past a level with an ASI/subclass/spell pick no
  longer silently applies a fixed default regardless of Plan Levels mode —
  every unresolved choice now queues as a real prompt at battle start,
  before wave 1.
- **Every remaining click-to-cycle button replaced with a real list
  picker** (D-164): Map Builder's Width/Height, Settings' Game Speed/
  Volume controls, Character Creation's Signature Action/Starting Level/
  Party Size/Difficulty/Team Level, and Free Play's/Browse Shared Maps'
  Difficulty all now open a full-screen list instead of cycling — the
  D-147 picker primitive was lifted into a shared `uiTheme.ts` helper to
  make this possible. True binary toggles (Mute, ability score method) and
  the mid-battle Game Speed keyboard hotkey were deliberately left as-is.

Tests: 1349 → 1348 (net -1: +26 in `levelUpPlanSystem.test.ts` rewritten to
assert unresolved-not-defaulted, -1 removed with the now-dead `nextVolume`,
`mainMenuLayout.test.ts` updated to the new corner-control region).
Typecheck, all 1348 tests, and the production build (126 modules,
unchanged) all pass. No browser available in this environment — every
visual/layout fix here needs Kevin's own pass, especially D-162 (see
KI-113).
